...
uint16_t NetworkServer::start(uint16_t port, bool loopbackOnly)
{
	const auto address = loopbackOnly ? boost::asio::ip::address_v4::loopback() : boost::asio::ip::address_v4::any();
	acceptor = std::make_shared<NetworkAcceptor>(context, boost::asio::ip::tcp::endpoint(address, port));
...
void NetworkServer::connectionAccepted(std::shared_ptr<NetworkSocket> upcomingConnection, const boost::system::error_code & ec)
{
	if(ec)
	{
		throw std::runtime_error("Something wrong during accepting: " + networkErrorMessage(ec));
	}
