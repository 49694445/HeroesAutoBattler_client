...
{
	// Empty means no server authority (ordinary maps and local training tools).
	const auto index = static_cast<size_t>(EGameSettings::ARENA_COMBAT_AI);
	baseSettings[index].String() = "";
	actualSettings[index].String() = "";
}
GameSettings::~GameSettings() = default;

const std::vector<GameSettings::SettingOption> GameSettings::settingProperties = {
		{EGameSettings::ARENA_COMBAT_AI, "arena", "combatAI"},
		...
		{EGameSettings::CREATURES_ADDITIONAL_WEEKLY_GROWTH_SPECIAL_WEEK,  "creatures", "additionalWeeklyGrowthSpecialWeek"    },
