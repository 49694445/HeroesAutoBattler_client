...
static const battle::Unit * takeOneUnit(battle::Units & allUnits, const int turn, BattleSide & sideThatLastMoved, const BattleSide initialTieBreakingSide, int phase)
{
	...
	for(size_t i = 0; i < allUnits.size(); i++)
	{
		...
		switch(phase)
		{
		case BattlePhases::NORMAL: // Faster first, initial tie priority, higher slot first
			...
			{
				if(sideThatLastMoved == BattleSide::NONE && turn <= 0 && currentUnit->unitSide() == initialTieBreakingSide
					&& !(returnedUnit->unitSide() == currentUnit->unitSide() && returnedUnit->unitSlot() < currentUnit->unitSlot())) // Initial turn priority
				{
					returnedUnit = currentUnit;
					currentUnitIndex = i;
				}
		...
		case BattlePhases::WAIT:
			if(returnedUnit == nullptr || currentUnitInitiative < returnedUnitInitiative)
			{
				...
				&& ((sideThatLastMoved == BattleSide::NONE && turn <= 0 && currentUnit->unitSide() == initialTieBreakingSide)
					|| (sideThatLastMoved != BattleSide::NONE && currentUnit->unitSide() != sideThatLastMoved))
				&& !(returnedUnit->unitSide() == currentUnit->unitSide() && returnedUnit->unitSlot() < currentUnit->unitSlot())) // Initial priority or alternate equal speeds units
			{
				returnedUnit = currentUnit;
				currentUnitIndex = i;
			}
...
void CBattleInfoCallback::battleGetTurnOrder(std::vector<battle::Units> & turns, const size_t maxUnits, const int maxTurns, const int turn, BattleSide sideThatLastMoved) const
{
	...
	const BattleSide initialTieBreakingSide = battle->getInitiativeTieBreakingSide();
	const bool preserveArenaInitialTieAfterSiege = battle->usesArenaInitiativeTieBreaking();
	...
	if(activeUnit)
	{
		...
		if(turn <= 0 && sideThatLastMoved == BattleSide::NONE
			&& !(preserveArenaInitialTieAfterSiege && activeUnit->battleQueuePhase(turn) == BattlePhases::SIEGE))
	...
	while(!turnsIsFull() && phase < BattlePhases::NUMBER_OF_PHASES)
	{
		...
		{
			currentUnit = takeOneUnit(phases[phase], actualTurn, sideThatLastMoved, initialTieBreakingSide, phase);
			if(!currentUnit)
			{
				phase++;
			}
