...
BattleSide BattleInfo::getInitiativeTieBreakingSide() const
{
	// Arena rounds keep red (player 0) on the left in ordinary battles, but
	// siege battles may reverse attacker/defender. Resolve the rule by player
	// color so it remains independent of the battle orientation.
	const auto * mapHeader = cb->getMapHeader();
	if(!mapHeader || !mapHeader->battleOnly)
		return BattleSide::ATTACKER;

	const bool redIsAttacker = getSidePlayer(BattleSide::ATTACKER) == PlayerColor(0);
	const bool redMovesFirst = cb->getDate(Date::DAY) % 2 != 0;

	return redMovesFirst == redIsAttacker ? BattleSide::ATTACKER : BattleSide::DEFENDER;
}

bool BattleInfo::usesArenaInitiativeTieBreaking() const
{
	const auto * mapHeader = cb->getMapHeader();
	return mapHeader && mapHeader->battleOnly;
}
