...
public:
	...
	/// Side that starts a cross-side initiative tie at the beginning of a battle queue.
	/// The default preserves the original attacker-first behavior.
	virtual BattleSide getInitiativeTieBreakingSide() const
	{
		return BattleSide::ATTACKER;
	}

	/// True only for battle-only arena battles that override the initial tie side.
	virtual bool usesArenaInitiativeTieBreaking() const
	{
		return false;
	}
};
