...
public:
	...
	bool replayAllowed;
	/// Local-only ArenaBattleProbe seed. A negative value means ordinary battle.
	int32_t arenaProbeSeed = -1;
	/// Stable local probe identity used to join client-side controller decisions.
	std::string arenaProbeScenarioId;
	...
	BattleSide getInitiativeTieBreakingSide() const override;
	bool usesArenaInitiativeTieBreaking() const override;
