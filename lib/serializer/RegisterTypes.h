...
void registerTypes(Serializer &s)
{
	...
	s.template registerType<SetArenaHealth>(265);
	s.template registerType<ArenaDeploymentStarted>(266);
	s.template registerType<ArenaDeploymentReady>(267);
	s.template registerType<SetArenaCreatureExperience>(268);
	s.template registerType<SetArenaRecruitState>(269);
	s.template registerType<SetArenaSummonsRemaining>(270);
	s.template registerType<ArenaRecruitAction>(271);
	// These IDs are a single flat namespace across every pack, not per-section.
	// LobbyAuthRequest/Result were first given 259/260 because the lobby block
	// happened to end at 258 - but those two were already taken by
	// UnitAdjacentLimiter and AdvInterfaceReady further down. Duplicate IDs
	// corrupt the type map, so packs arrive as the wrong type and the client
	// crashes somewhere unrelated. Always take the next number after the global
	// maximum.
	s.template registerType<LobbyAuthRequest>(272);
	s.template registerType<LobbyAuthResult>(273);
	s.template registerType<LobbySetPlayerReady>(274);
	s.template registerType<LobbyClientAbandonGame>(275);
	s.template registerType<ArenaDisconnectWaitState>(276);
	s.template registerType<ArenaTownVisitRequest>(277);
	s.template registerType<SetArenaTownVisitState>(278);
	s.template registerType<ArenaBuildLevelQuoteRequest>(279);
	s.template registerType<SetArenaBuildLevelState>(280);
	s.template registerType<ArenaBuildDwellingWithLevel>(281);
	s.template registerType<SetArenaTownSummonState>(282);
	s.template registerType<ArenaSummonFromTown>(283);
	s.template registerType<SetArenaMainHero>(284);
	s.template registerType<SetArenaTownBuildGateState>(285);
	s.template registerType<SetArenaBattleCreatureProgress>(286);
	s.template registerType<SetArenaHeroLevelUpChoiceTimeout>(287);
	s.template registerType<SetArenaBattleResultTimeout>(288);
	s.template registerType<ArenaCommandChannelClientRequest>(289);
	s.template registerType<ArenaCommandChannelClientResponse>(290);
	s.template registerType<ArenaArtifactDraftSelection>(292);
	s.template registerType<ArenaArtifactDraftPick>(293);
	s.template registerType<LobbyServerRedirect>(294);
	s.template registerType<ArenaDailyResourceGrant>(295);
	s.template registerType<ArenaSetMainHeroRequest>(296);
	s.template registerType<ArenaDeploymentPlanCommit>(297);
	s.template registerType<ArenaNativeProbeRequest>(298);
	s.template registerType<ArenaNativeProbeResult>(299);
	s.template registerType<SetStackArenaInitialUnit>(291);
}
