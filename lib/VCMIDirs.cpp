...
{
	// Native AI child processes share read-only game assets, but never the
	// player's mutable configuration, cache, logs or save directory.
	wchar_t nativeRoot[32768];
	const auto nativeLength=GetEnvironmentVariableW(L"VCMI_NATIVE_SIM_ROOT",nativeRoot,32768);
	if(nativeLength>0 && nativeLength<32768)
	{
		const bfs::path root(nativeRoot);
		if(!root.is_absolute()) throw std::runtime_error("native simulation root must be absolute");
		if(key=="userConfigPath") return root/"config";
		if(key=="userCachePath") return root/"cache";
		if(key=="userLogsPath") return root/"logs";
		if(key=="userSavePath")
		{
			wchar_t saves[32768];
			const auto length=GetEnvironmentVariableW(L"VCMI_NATIVE_SIM_SAVES",saves,32768);
			if(!length || length>=32768 || !bfs::path(saves).is_absolute()) throw std::runtime_error("native simulation saves must be absolute");
			return bfs::path(saves);
		}
	}
...
std::vector<bfs::path> VCMIDirsAndroid::dataPaths() const
{
	...
	// Managed gameplay resources override externally imported copies.
	return {userDataPath(), internalPath, bfs::path(internalPath) / "hab-bundle"};
#else
	return {
		internalPath,
		userDataPath(),
