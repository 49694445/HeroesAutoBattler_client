...
ModsPresetState::ModsPresetState()
{
	...
	// Preserve user presets, but run this product with its tested gameplay preset.
	JsonNode fixed(JsonPath::builtin("config/habFixedMods.json"));
	const bool chinese = settings["general"]["language"].String() == "chinese";
	if(chinese)
		fixed["mods"].Vector().emplace_back("chinese-translation");
	fixed["settings"]["hota"]["mainmenu.buttonschinese"].Bool() = chinese;
	if(CResourceHandler::get()->existsResource(getModDefinitionFile("hab-lan-test")))
		fixed["mods"].Vector().emplace_back("hab-lan-test");
	modConfig["presets"]["HeroesAutoBattler"] = fixed;
	modConfig["activePreset"].String() = "HeroesAutoBattler";
#endif
}
