...
void SettingsStorage::invalidateNode(const std::vector<std::string> &changedPath)
{
	...
	// Session settings are process-local by definition. They are stripped from
	// the serialized document below, so writing the file for a session-only
	// change is both redundant and unsafe when multiple clients run in parallel.
	if(!changedPath.empty() && changedPath.front() == "session")
		return;
