...
public:
	using TPreRandomizationCallback = std::function<void(CGameState &)>;
	...
	void init(const IMapService * mapService, StartInfo * si, IGameRandomizer & gameRandomizer, Load::ProgressAccumulator &, bool allowSavingRandomMap = true, const TPreRandomizationCallback & preRandomizationCallback = {});
...
/// Creates a fully detached state for simulations without instantiating the
/// serialization graph in callers across the VCMI library boundary.
DLL_LINKAGE std::shared_ptr<CGameState> cloneGameStateForSimulation(const CGameState & source);
