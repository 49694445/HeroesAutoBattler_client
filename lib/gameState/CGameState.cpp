...
{
	// A simulation clone must rebuild the same local bonus-system topology as a
	// savegame load.  The generic memory deep-copy path intentionally leaves
	// loadingGamestate disabled; that preserves serialized bonus sources but does
	// not restore parentsToPropagate/children.  Mutable detached branches then
	// report invalid detach operations as soon as a hero enters a virtual town.
	CMemorySerializer memory;
	memory.oser & &source;
	memory.iser.cb = const_cast<CGameState *>(&source);
	memory.iser.loadingGamestate = true;
	std::shared_ptr<CGameState> result;
	memory.iser & result;
	return result;
}
...
void CGameState::init(const IMapService * mapService, StartInfo * si, IGameRandomizer & gameRandomizer, Load::ProgressAccumulator & progressTracking, bool allowSavingRandomMap, const TPreRandomizationCallback & preRandomizationCallback)
{
	assert(services);
	...
	if(preRandomizationCallback)
		preRandomizationCallback(*this);
