...
void GameStatePackVisitor::visitSetArenaHealth(SetArenaHealth & pack)
{
	...
	if(pack.mode == ChangeValueMode::ABSOLUTE)
		player->arenaHealth = pack.health;
	else
		player->arenaHealth += pack.health;
}

void GameStatePackVisitor::visitSetArenaCreatureExperience(SetArenaCreatureExperience & pack)
{
	assert(pack.player.isValidPlayer());
	auto * player = gs.getPlayerState(pack.player);
	if(!player)
	...
	auto & value = player->arenaCreatureExperience[pack.creature];
	if(pack.mode == ChangeValueMode::ABSOLUTE)
		value = pack.experience;
	else
		value += pack.experience;
}

void GameStatePackVisitor::visitSetArenaRecruitState(SetArenaRecruitState & pack)
{
	assert(pack.player.isValidPlayer());
	auto * player = gs.getPlayerState(pack.player);
	if(!player)
	...
	// The server always sends the whole record, so every field is absolute.
	player->arenaRecruitThreshold[pack.creature] = pack.threshold;
	if(pack.levelLocked)
		player->arenaRecruitLevel[pack.creature] = pack.level;
	else
		player->arenaRecruitLevel.erase(pack.creature);
	player->arenaRecruitCount[pack.creature] = pack.recruitCount;
	player->arenaRecruitCost[pack.creature] = pack.cost;
	player->arenaRecruitAmount[pack.creature] = pack.amount;

	if(pack.unlocked)
		player->arenaUnlockedCreatures.insert(pack.creature);
	else
		player->arenaUnlockedCreatures.erase(pack.creature);
}

void GameStatePackVisitor::visitSetArenaSummonsRemaining(SetArenaSummonsRemaining & pack)
{
	assert(pack.player.isValidPlayer());
	auto * player = gs.getPlayerState(pack.player);
	if(!player)
	...
	if(pack.mode == ChangeValueMode::ABSOLUTE)
		player->arenaSummonsRemaining = pack.summons;
	else
		player->arenaSummonsRemaining += pack.summons;
}

void GameStatePackVisitor::visitSetArenaMainHero(SetArenaMainHero & pack)
{
	if(auto * player = gs.getPlayerState(pack.player, false))
		player->arenaMainHero = pack.mainHero;
}
...
void GameStatePackVisitor::visitHeroRecruited(HeroRecruited & pack)
{
	...
	if(pack.heroInstanceId.hasValue())
		h->id = pack.heroInstanceId;
	CGTownInstance *t = gs.getTown(pack.tid);
	PlayerState *p = gs.getPlayerState(pack.player);
	...
	// The server applies this mutable pack before sending it. Preserve the ID it
	// allocated so clients revealing a privately recruited hero cannot allocate
	// a different object ID and then crash on queued artifact/army operations.
	pack.heroInstanceId = h->id;
...
{
	if(!pack.instanceId.hasValue())
		pack.instanceId = gs.getMap().nextArtifactInstanceId();
	auto art = gs.getMap().createArtifact(pack.artId, pack.spellId, pack.instanceId);
...
void GameStatePackVisitor::visitSetStackArenaInitialUnit(SetStackArenaInitialUnit & pack)
{
	auto * army = gs.getArmyInstance(pack.army);
	if(!army)
		throw std::runtime_error("SetStackArenaInitialUnit: invalid army object " + std::to_string(pack.army.getNum()) + ", possible game state corruption.");
	auto * stack = army->getStackPtr(pack.slot);
	if(!stack)
		throw std::runtime_error("SetStackArenaInitialUnit: invalid stack slot " + std::to_string(pack.slot.getNum()) + ", possible game state corruption.");
	stack->setArenaInitialUnit(pack.initialUnit);
}
...
void GameStatePackVisitor::visitBattleStart(BattleStart & pack)
{
	...
	if (pack.info->getDefendedTown() && pack.info->getSide(BattleSide::DEFENDER).heroID.hasValue())
	{
		...
		// A defended town can also be supplied for an Arena siege where the
		// defending hero is outside that town. Only a hero actually stationed in
		// this town is attached to townAndVis and needs this hand-off.
		if (hero && (town->getVisitingHero() == hero || town->getGarrisonHero() == hero))
		{
...
void GameStatePackVisitor::visitBattleEnded(BattleEnded & pack)
{
	...
	assert(battleIter != gs.currentBattles.end());
	const auto & endingBattle = **battleIter;
	if(endingBattle.arenaProbeSeed >= 0)
	{
		// Probe army objects intentionally survive for the resident process because
		// clients may still hold references after BattleEnded. Detach the completed
		// pair from its adventure-map owner bonus graph on every game state before
		// destroying BattleInfo; otherwise surviving stacks export owner-wide
		// bonuses into later scenarios.
		for(const auto side : {BattleSide::ATTACKER, BattleSide::DEFENDER})
			if(auto * army = const_cast<CArmedInstance *>(endingBattle.battleGetArmyObject(side)))
				army->detachFromBonusSystem(gs);
	}
	gs.currentBattles.erase(battleIter);
}
