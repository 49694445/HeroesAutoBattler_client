...
//commands
	...
	void arenaDeploymentReady() override;
	void arenaNativeProbe(int32_t day, const std::string & matchId, const std::string & authorityId,
		const std::string & requestId, const std::string & payload);
	void arenaDeploymentPlanCommit(int32_t day, QueryID draftQueryID,
		...
		const std::vector<std::string> & atomicActionIds) override;
	void arenaArtifactDraftPick(QueryID queryID, int32_t candidateIndex) override;
	void arenaRecruit(CreatureID creature);
	void arenaTownVisit(ObjectInstanceID town, int32_t action, uint32_t expectedRevision = 0, bool planningOnly = false);
	void arenaBuildLevelQuote(ObjectInstanceID town, BuildingID building);
	void arenaBuildDwellingWithLevel(ObjectInstanceID town, BuildingID building, CreatureID creature, int32_t level, uint32_t expectedRevision);
	void arenaSummonFromTown(ObjectInstanceID town, CreatureID creature, uint32_t expectedVisitRevision, uint32_t expectedSummonRevision);
