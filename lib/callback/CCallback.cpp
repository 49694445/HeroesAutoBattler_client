...
void CCallback::arenaDeploymentReady()
{
	logGlobal->trace("Player %d finished arena deployment.", getPlayerID()->getNum());
	ArenaDeploymentReady pack;
	sendRequest(pack);
}

void CCallback::arenaNativeProbe(int32_t day,const std::string & matchId,const std::string & authorityId,
	const std::string & requestId,const std::string & payload)
{
	ArenaNativeProbeRequest pack;
	pack.day=day; pack.matchId=matchId; pack.authorityId=authorityId; pack.requestId=requestId; pack.payload=payload;
	sendRequest(pack);
}

void CCallback::arenaDeploymentPlanCommit(int32_t day, QueryID draftQueryID,
	const std::string & selectedPlanStableId,
	const std::vector<std::string> & selectedReinforcementStableIds,
	int32_t abandonedReinforcementSlots,
	const std::vector<std::string> & atomicActionIds)
{
	ArenaDeploymentPlanCommit pack;
	pack.day = day;
	pack.draftQueryID = draftQueryID;
	pack.selectedPlanStableId = selectedPlanStableId;
	pack.selectedReinforcementStableIds = selectedReinforcementStableIds;
	pack.abandonedReinforcementSlots = abandonedReinforcementSlots;
	pack.atomicActionIds = atomicActionIds;
	sendRequest(pack);
}

void CCallback::arenaArtifactDraftPick(QueryID queryID, int32_t candidateIndex)
{
	ArenaArtifactDraftPick pack;
	pack.queryID = queryID;
	pack.candidateIndex = candidateIndex;
	sendRequest(pack);
}

void CCallback::arenaRecruit(CreatureID creature)
{
	ArenaRecruitAction pack;
	pack.action = ArenaRecruitAction::EAction::RECRUIT;
	pack.creature = creature;
	sendRequest(pack);
}

void CCallback::arenaTownVisit(ObjectInstanceID town, int32_t action, uint32_t expectedRevision, bool planningOnly)
{
	ArenaTownVisitRequest pack;
	pack.action = static_cast<ArenaTownVisitRequest::EAction>(action);
	pack.town = town;
	pack.expectedRevision = expectedRevision;
	pack.planningOnly = planningOnly;
	sendRequest(pack);
}

void CCallback::arenaBuildLevelQuote(ObjectInstanceID town, BuildingID building)
{
	ArenaBuildLevelQuoteRequest pack;
	pack.town = town;
	pack.building = building;
	sendRequest(pack);
}

void CCallback::arenaBuildDwellingWithLevel(ObjectInstanceID town, BuildingID building, CreatureID creature, int32_t level, uint32_t expectedRevision)
{
	ArenaBuildDwellingWithLevel pack;
	pack.town = town;
	pack.building = building;
	pack.creature = creature;
	pack.level = level;
...
void CCallback::arenaSummonFromTown(ObjectInstanceID town, CreatureID creature, uint32_t expectedVisitRevision, uint32_t expectedSummonRevision)
{
	ArenaSummonFromTown pack;
	pack.town = town;
	pack.creature = creature;
	pack.expectedVisitRevision = expectedVisitRevision;
	pack.expectedSummonRevision = expectedSummonRevision;
	sendRequest(pack);
}
...
bool CCallback::buildBuilding(const CGTownInstance *town, BuildingID buildingID)
{
	...
	const auto buildState = canBuildStructure(town, buildingID);
	const bool arenaDynamicFort = getMapHeader()->battleOnly
		&& buildingID == BuildingID::FORT
		&& buildState == EBuildingState::NO_RESOURCES;
	if(buildState != EBuildingState::ALLOWED && !arenaDynamicFort)
		return false;
...
{
	// SaveLocalState and SaveGame travel over the same ordered connection. Flush
	// client-private UI / battle-AI state first so the server snapshots that exact
	// state without making plugin internals part of authoritative game rules.
	const auto localState = prepareLocalStateForSave();
	if(!localState.isNull())
