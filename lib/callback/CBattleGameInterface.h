...
static constexpr int AI_INTERFACE_VER = 2;
...
public:
	...
	/// Optional plugin-private save data. The default keeps old AIs source- and
	/// save-compatible without exposing their state to server game rules.
	virtual void saveLocalState(JsonNode & dest) const {};
	virtual void loadLocalState(const JsonNode & source) {};
