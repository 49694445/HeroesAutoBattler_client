...
#ifdef STATIC_AI
// Static AI headers live outside the library namespace, like their own
// translation units. Do not depend on an adventure AI header importing it.
...
{
	if(libpath.stem() == "libEmptyAI" && methodName == "GetNewArenaAI")
		return std::make_shared<CServerDrivenArenaAI>();
#ifdef ENABLE_NULLKILLER2_AI
...
std::shared_ptr<CGlobalAI> CDynLibHandler::getNewServerDrivenArenaAI()
{
	return createAnyAI<CGlobalAI>("EmptyAI", "GetNewArenaAI");
}
