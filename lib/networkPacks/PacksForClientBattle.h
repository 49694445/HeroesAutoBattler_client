...
struct DLL_LINKAGE BattleResult : public Query
{
	...
	/// Arena-only actual health reduction settled by the server for this battle.
	BattleSideArray<int32_t> arenaHealthDamage{0,0};
...
{
	struct ArenaDemonSummon
	{
		uint32_t consumedUnit = 0;
	...
	// Server-only metadata. DemonSummon fills this before the pack reaches the
	// server environment so Arena can persist the conversion after restoring the
	// pre-battle army snapshot. It is intentionally not serialized to clients.
	std::vector<ArenaDemonSummon> arenaDemonSummons;
