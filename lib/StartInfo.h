...
struct ClientPlayer
{
	...
	/// Arena: this participant has confirmed they are done setting up. The server
	/// starts the game once every participant is ready - there is no host who
	/// starts it on everyone's behalf. Written by the server only; it reaches the
	/// clients as part of the lobby state, so no separate pack carries it.
	bool ready = false;
