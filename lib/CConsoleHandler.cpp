...
#ifdef _MSC_VER
	case 0xE06D7363: // not in winnt.h - this is how MSVC ships a C++ throw
		return "C++ EXCEPTION";
#endif
	default:
...
// A C++ exception that nobody caught reaches this filter as a plain SEH record
// with this code and no hint of what was thrown. Decoding it is the difference
// between a log that names the problem and a log that just stops - see the
// startup failures of 2026-08-11, where the only visible symptom was the log
// ending mid-sentence.
static constexpr DWORD MSVC_CXX_EXCEPTION = 0xE06D7363; // 0xE0000000 | 'msc'
static constexpr ULONG_PTR MSVC_CXX_MAGIC_MIN = 0x19930520;
static constexpr ULONG_PTR MSVC_CXX_MAGIC_MAX = 0x19930522;

// The metadata MSVC hangs off a throw. There is no public header for it
// (ehdata.h is internal), so it is spelled out here. Every "rva" is an offset
// from the throwing module's base on x64 and an absolute address on x86, which
// is why the base is added in one place and is simply zero for 32 bit builds.
namespace
{
	struct MsvcTypeDescriptor
	{
		const void * vftable;
		void * spare;
		char name[1]; // decorated, e.g. ".?AVruntime_error@std@@"
	};
	...
	struct MsvcCatchableType
	{
		...
		int32_t baseDisplacementMdisp;
		int32_t baseDisplacementPdisp;
		int32_t baseDisplacementVdisp;
	...
	struct MsvcThrowInfo
	{
		uint32_t attributes;
		uint32_t unwind;
		uint32_t forwardCompat;
		uint32_t catchableTypeArray;
	};
}
...
// Everything below reads memory of a process that is already broken, so it all
// sits behind __try: a diagnostic must never be the reason the crash dump goes
// missing. No C++ object with a destructor may live in here - __try forbids it.
static bool describeThrownException(const EXCEPTION_RECORD * record, char * typeName, size_t typeNameSize, char * reason, size_t reasonSize)
{
	__try
	{
		if (record->NumberParameters < 3)
			return false;
		if (record->ExceptionInformation[0] < MSVC_CXX_MAGIC_MIN || record->ExceptionInformation[0] > MSVC_CXX_MAGIC_MAX)
			return false;
		...
		// A rethrow carries no object and no metadata of its own
		if (object == nullptr || throwInfo == nullptr)
			return false;

#ifdef _WIN64
		const uintptr_t base = record->NumberParameters >= 4 ? record->ExceptionInformation[3] : 0;
#else
		const uintptr_t base = 0;
#endif
		if (throwInfo->catchableTypeArray == 0)
			return false;

		const auto * types = reinterpret_cast<const MsvcCatchableTypeArray *>(base + throwInfo->catchableTypeArray);

		// Entry zero is the thrown type itself, the rest are its base classes
		for (int32_t i = 0; i < types->typeCount; ++i)
		{
			const auto * catchable = reinterpret_cast<const MsvcCatchableType *>(base + types->types[i]);
			const auto * descriptor = reinterpret_cast<const MsvcTypeDescriptor *>(base + catchable->typeDescriptor);

			if (i == 0)
				copyText(typeName, typeNameSize, descriptor->name);

			// Ask std::exception itself for the message rather than guessing at
			// the layout the standard library happens to use for it today. The
			// displacement turns the thrown object into that base subobject;
			// only the non-virtual case can occur for std::exception.
			if (std::strcmp(descriptor->name, ".?AVexception@std@@") == 0 && catchable->baseDisplacementPdisp < 0)
			{
				const auto * asException = reinterpret_cast<const std::exception *>(object + catchable->baseDisplacementMdisp);
				const char * text = asException->what();
				if (text != nullptr)
...
// ".?AVruntime_error@std@@" reads as "std::runtime_error". Scopes are stored
// innermost first, so they come back reversed.
static std::string readableTypeName(const std::string & decorated)
{
	const bool isClass = decorated.compare(0, 4, ".?AV") == 0;
	const bool isStruct = decorated.compare(0, 4, ".?AU") == 0;

	if (decorated.size() < 7 || (!isClass && !isStruct) || decorated.compare(decorated.size() - 2, 2, "@@") != 0)
		return decorated;

	std::string body = decorated.substr(4, decorated.size() - 6); // drop ".?AV" and trailing "@@"
	std::vector<std::string> scopes;
	boost::split(scopes, body, boost::is_any_of("@"));
	std::reverse(scopes.begin(), scopes.end());
	return boost::algorithm::join(scopes, "::");
}
...
LONG WINAPI onUnhandledException(EXCEPTION_POINTERS* exception)
{
	...
	logGlobal->error("Exception code: 0x%08x (%s)", einfo->ExceptionCode, exceptionName(einfo->ExceptionCode));
...
#ifdef _MSC_VER
	else if (einfo->ExceptionCode == MSVC_CXX_EXCEPTION)
	{
		// This filter runs instead of std::terminate, not before it, so
		// onTerminate below never gets to report an escaping exception on
		// Windows. Without the next few lines the log says nothing at all
		// about the cause.
		char typeName[256] = {};
		char reason[2048] = {};

		if (describeThrownException(einfo, typeName, sizeof(typeName), reason, sizeof(reason)))
		{
			logGlobal->error("Uncaught exception of type %s", readableTypeName(typeName));

			if (reason[0] != '\0')
				logGlobal->error("Reason: %s", reason);
			else
				logGlobal->error("Reason: unavailable, the thrown type does not derive from std::exception");
		}
		else
		{
			logGlobal->error("Uncaught C++ exception, but its description could not be read");
		}
	}
#endif
	...
	// The filter runs before the stack is unwound, so the frames that threw are
	// still here. Logged before the dump is written because createMemoryDump
	// ends in a modal message box.
