...
void CMap::addNewObject(std::shared_ptr<CGObjectInstance> obj)
{
	...
	if(obj->id.getNum() < static_cast<int32_t>(objects.size()) && objects.at(obj->id.getNum()) != nullptr)
		throw std::runtime_error("Invalid object instance id");

	// Private arena objects are revealed later with their server-authoritative
	// IDs. Keep gaps for objects that this client is not allowed to know yet.
	if(obj->id.getNum() > static_cast<int32_t>(objects.size()))
		objects.resize(static_cast<size_t>(obj->id.getNum()));
...
CArtifactInstance * CMap::createArtifact(const ArtifactID & artID, const SpellID & spellId, const ArtifactInstanceID & instanceId)
{
	if(!instanceId.hasValue())
		return createArtifact(artID, spellId);
	if(instanceId.getNum() < static_cast<int32_t>(artInstances.size()))
		throw std::runtime_error("Artifact instance ID was already allocated");

	artInstances.resize(static_cast<size_t>(instanceId.getNum()));
	auto * artifact = createArtifact(artID, spellId);
	if(artifact->getId() != instanceId)
		throw std::runtime_error("Failed to allocate server-authoritative artifact instance ID");
	return artifact;
}

ArtifactInstanceID CMap::nextArtifactInstanceId() const
{
	return ArtifactInstanceID(static_cast<int32_t>(artInstances.size()));
}
