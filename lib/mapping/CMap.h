...
public:
	...
	/// Creates an artifact at the ID allocated by the server, padding hidden
	/// private arena artifacts with null registry entries when necessary.
	CArtifactInstance * createArtifact(const ArtifactID & artId, const SpellID & spellId, const ArtifactInstanceID & instanceId);
	ArtifactInstanceID nextArtifactInstanceId() const;
