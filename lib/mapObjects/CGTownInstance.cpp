...
void CGTownInstance::pickRandomObject(IGameRandomizer & gameRandomizer)
{
	...
	{
		ID = MapObjectID::TOWN;
		if(forcedRandomFaction)
			subID = *forcedRandomFaction;
		else
			subID = randomizeFaction(gameRandomizer.getDefault());
		forcedRandomFaction.reset();
	}
...
void CGTownInstance::setForcedRandomFaction(FactionID faction)
{
	if(ID != MapObjectID::RANDOM_TOWN)
		throw std::logic_error("Forced random faction can only be set for a random town");
	if(!faction.toFaction() || !faction.toFaction()->hasTown())
		throw std::invalid_argument("Forced random faction must identify a playable town faction");

	forcedRandomFaction = faction;
}
