...
public:
	...
	struct CreatureSpecialtyBonuses
	{
		int32_t attack = 0;
		int32_t defense = 0;
		int32_t speed = 0;
	...
	/// Resolve this hero's level-scaled, creature-limited specialty bonuses by
	/// the same updater and rounding rules used by the bonus system.
	CreatureSpecialtyBonuses getCreatureSpecialtyBonuses(CreatureID creature) const;

	/// Whether this hero type owns a creature-limited specialty for this exact
	/// creature or one of the specialty's allowed upgrades. This intentionally
	/// identifies the limiter rather than inferring it from a non-zero numeric
	/// combat projection: fixed specialties may affect damage or other effects.
	bool hasCreatureSpecialty(CreatureID creature) const;
