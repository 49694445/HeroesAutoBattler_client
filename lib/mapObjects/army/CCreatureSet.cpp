...
void CCreatureSet::joinStack(const SlotID & slot, std::unique_ptr<CStackInstance> stack)
{
	...
	// A stack remains an untouched arena starting unit only when both merged
	// parts still have that provenance.
	getStackPtr(slot)->setArenaInitialUnit(getStackPtr(slot)->isArenaInitialUnit() && stack->isArenaInitialUnit());
	//TODO move stuff
...
std::unique_ptr<CStackInstance> CCreatureSet::splitStack(const SlotID & slot, TQuantity toSplit)
{
	...
	newStack->setArenaInitialUnit(currentStack->isArenaInitialUnit());
