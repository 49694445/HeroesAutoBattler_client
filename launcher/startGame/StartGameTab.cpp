...
void StartGameTab::refreshState()
{
	...
	for(const auto * name : {"scrollArea", "labelTitleModPreset", "buttonUpdateCheck", "buttonOpenDownloads", "buttonOpenChangelog",
		"labelUpdateNotFound", "labelUpdateAvailable", "buttonUpdateMods", "buttonUpdateModsHelp",
		"buttonInstallTranslation", "buttonInstallTranslationHelp", "buttonActivateTranslation", "buttonActivateTranslationHelp",
		"buttonInstallHdEdition", "buttonInstallHdEditionHelp", "buttonChroniclesHelp"})
		if(auto * widget = findChild<QWidget *>(name)) widget->hide();
	ui->labelChronicles->setText(settings["general"]["language"].String() == "chinese"
		? QString::fromUtf8("已内置 HotA、战斗 AI 和中文资源。语言可在设置中选择。")
		: QStringLiteral("HotA, combat AI and Chinese resources are included. Choose your language in Settings."));
	ui->labelChronicles->show();
	ui->labelTitleEngine->setText("HeroesAutoBattler");
#endif
