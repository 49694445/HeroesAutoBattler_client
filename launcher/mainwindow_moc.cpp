...
void MainWindow::load()
{
	...
	{ Settings node = settings.write["launcher"]["updateOnStartup"]; node->Bool() = false; }
	{ Settings node = settings.write["launcher"]["defaultRepositoryEnabled"]; node->Bool() = false; }
	{ Settings node = settings.write["launcher"]["extraRepositoryEnabled"]; node->Bool() = false; }
#endif
}
...
#if defined(VCMI_ANDROID) && defined(VCMI_REMOTE_CLIENT_ONLY)
	if(settings["general"]["language"].String() != "chinese" && settings["general"]["language"].String() != "english")
		{ Settings node = settings.write["general"]["language"]; node->String() = "english"; }
#endif
...
#if defined(VCMI_ANDROID) && defined(VCMI_REMOTE_CLIENT_ONLY)
	if(realFilePath.endsWith(".json", Qt::CaseInsensitive) ||
		(realFilePath.endsWith(".zip", Qt::CaseInsensitive) && QFileInfo(realFilePath).fileName() != "hab-lan-test.zip"))
	{
		QMessageBox::information(this, tr("HeroesAutoBattler"), tr("Required game content is included. Choose the game language in Settings."));
		return;
	}
#endif
