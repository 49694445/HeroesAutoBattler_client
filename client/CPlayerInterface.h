...
struct CPathsInfo;
	...
	struct SetArenaBattleCreatureProgress;
...
{
	struct ArenaBattleCreatureProgress
	{
		PlayerColor player;
		CreatureID creature;
		int32_t level;
		int64_t currentExperience;
		int64_t maximumExperience;
	};
	struct ArenaTownSummonState
	{
		ObjectInstanceID town;
		BuildingID building;
		CreatureID creature;
		ObjectInstanceID mainHero;
		uint32_t visitRevision;
		uint32_t summonRevision;
		EArenaTownSummonStatus status;
		int32_t amount;
		TResources cost;
		std::string reason;
		bool pending = false;
...
public: // TODO: make private
	...
	std::atomic_bool arenaDeploymentActive = false;
	// The deployment UI is not interactive while the opening resource notice or
	// reinforcement choice is active. These flags also stop clicks from being
	// queued behind a busy server and arriving after the choice has opened.
	std::atomic_bool arenaDailyResourcePresentationActive = false;
	std::atomic_bool arenaArtifactDraftActive = false;
	std::atomic_int32_t arenaNextBattleTerrain = TerrainId::NONE;
	std::atomic_bool arenaNextBattleSiege = false;
	std::atomic_int32_t arenaLastOrdinaryBattleLoser = PlayerColor::NEUTRAL.getNum();
	std::atomic_int32_t arenaConsecutiveOrdinaryLosses = 0;
	std::atomic_int32_t arenaSiegeInterval = 1;
	// Arena virtual-town visit state is updated from private server packets. It is
	// deliberately kept client-side only as UI/session state; the server remains
	// authoritative for the actual visiting relationship.
	ObjectInstanceID arenaTownVisitTown;
	ObjectInstanceID arenaTownVisitHero;
	ObjectInstanceID arenaTownVisitPendingTown;
	uint32_t arenaTownVisitRevision = 0;
	bool arenaTownVisitActive = false;
	bool arenaTownVisitPending = false;
	ObjectInstanceID arenaTownBuildGateTown;
	uint32_t arenaTownBuildGateVisitRevision = 0;
	BuildingID arenaTownBuildGateRequired = BuildingID::NONE;
	bool arenaTownBuildGateLocked = false;
	bool arenaTownBuildGateValid = false;
	TResources arenaTownFortCost;
	BattleID arenaBattleProgressBattle = BattleID::NONE;
	int32_t arenaBattleProgressRound = -1;
	std::vector<ArenaBattleCreatureProgress> arenaBattleProgress;
	std::vector<ArenaTownSummonState> arenaTownSummons;
	uint32_t arenaTownSummonRevision = 0;
	std::map<QueryID, int32_t> arenaHeroLevelUpChoiceTimeouts;
	std::map<QueryID, int32_t> arenaBattleResultTimeouts;
	std::optional<ArenaHeroSnapshot> arenaPreviousOwnHero;
	std::optional<ArenaHeroSnapshot> arenaPreviousEnemyHero;

	void setArenaHeroLevelUpChoiceTimeout(QueryID queryID, int32_t remainingSeconds);
	void setArenaBattleResultTimeout(QueryID queryID, int32_t remainingSeconds);
	int32_t getArenaBattleResultTimeout(QueryID queryID) const;
...
public: // public interface for use by client via GAME->interface() access
	...
	/// Sends an arena ENTER request for owned towns, otherwise opens normally.
	void requestTownWindow(const CGTownInstance * town);
	bool requestArenaTownLeave(const CGTownInstance * town);
	bool requestArenaTownSwitch(const CGTownInstance * town);
	bool requestArenaMainHero(const CGHeroInstance * hero);
	bool isArenaTownVisit(const CGTownInstance * town) const;
	void applyArenaTownVisitState(const SetArenaTownVisitState & pack);
	void applyArenaTownBuildGateState(const SetArenaTownBuildGateState & pack);
	bool isArenaTownBuildGateLocked(const CGTownInstance * town, BuildingID building, BuildingID * requiredBuilding = nullptr) const;
	std::optional<TResources> getArenaTownFortCost(const CGTownInstance * town) const;
	void applyArenaBattleCreatureProgress(const SetArenaBattleCreatureProgress & pack);
	void clearArenaBattleCreatureProgress(BattleID battleID);
	std::optional<ArenaBattleCreatureProgress> getArenaBattleCreatureProgress(BattleID battleID, PlayerColor player, CreatureID creature) const;
	void applyArenaTownSummonState(const SetArenaTownSummonState & pack);
	std::optional<ArenaTownSummonState> getArenaTownSummonState(const CGTownInstance * town, BuildingID building, CreatureID creature) const;
	bool requestArenaTownSummon(const CGTownInstance * town, BuildingID building, CreatureID creature);
	void closeArenaTownWindow(ObjectInstanceID town);
	...
	void endNetwork();
	void setArenaDeploymentActive(bool active);
	void setArenaNextBattleTerrain(TerrainId terrain);
	TerrainId getArenaNextBattleTerrain() const;
	void setArenaNextBattleSiege(bool siege);
	bool isArenaNextBattleSiege() const;
	void setArenaLastOrdinaryBattleLoss(PlayerColor loser, int32_t count, int32_t siegeInterval);
	PlayerColor getArenaLastOrdinaryBattleLoser() const;
	int32_t getArenaConsecutiveOrdinaryLosses() const;
	int32_t getArenaSiegeInterval() const;
	void setArenaPreviousBattleHeroes(const std::optional<ArenaHeroSnapshot> & ownHero, const std::optional<ArenaHeroSnapshot> & enemyHero);
	const std::optional<ArenaHeroSnapshot> & getArenaPreviousOwnHero() const;
	const std::optional<ArenaHeroSnapshot> & getArenaPreviousEnemyHero() const;
