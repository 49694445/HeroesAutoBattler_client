...
void InputHandler::dispatchMainThread(const std::function<void()> & functor)
{
	...
	// NOTE: approach with dispatchedTasks container is a bit excessive and is
	// used mostly to prevent false-positive leaks in analyzers. Publish the task
	// before its wake-up event: pushing the SDL event first lets
	// the GUI thread consume it before dispatchedTasks contains the matching
	// functor, leaving that functor stranded until an unrelated later event.
