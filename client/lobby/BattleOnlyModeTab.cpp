...
BattleOnlyModeTab::BattleOnlyModeTab()
	...
	, legacySetupUi(!GAME->server().battleMode)
...
void BattleOnlyModeTab::prepareLobbyMap()
{
...
void BattleOnlyModeTab::startBattle()
{
	...
	extraOptions.unlimitedReplay = false; // Arena replay can settle the same round twice and deduct health twice.
	extraOptions.cheatsAllowed = false;
	GAME->server().setExtraOptionsInfo(extraOptions);

	// Create the standard adventure-map timer widget for arena deployment.
	// The server remains authoritative and replaces this display seed through TurnTimeUpdate.
	TurnTimerInfo arenaTurnTimer;
	arenaTurnTimer.turnTimer = 60 * 1000;
	GAME->server().setTurnTimerInfo(arenaTurnTimer);
