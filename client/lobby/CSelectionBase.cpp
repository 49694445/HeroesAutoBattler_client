	...
	: showChat(true)
{
	...
	else
	{
		...
		iconDifficulty = std::make_shared<CToggleGroup>(0);
		{
			...
			constexpr std::array difficultyPercent = {80, 100, 130, 160, 200};

			for(int i = 0; i < 5; i++)
			{
				auto button = std::make_shared<CToggleButton>(Point(110 + i * 32, 450), AnimationPath::builtin(difButns[i]),
					CButton::tooltipLocalized("vcmi.arena.difficulty." + std::to_string(difficultyPercent[i])));
...
void InfoCard::changeSelection()
{
	...
	for(const auto & p : GAME->server().playerNames)
	{
		...
		labelGroupPlayers->add(labelPosition.x, labelPosition.y, p.second.name + (p.second.ready ? " [READY]" : " [NOT READY]"));
	}
}
...
void InfoCard::showBattleOnlyRules()
{
	labelVictoryConditionText->setText(LIBRARY->generaltexth->translate("vcmi.lobby.battleOnlyModeVictoryCondition"));
	labelLossConditionText->setText(LIBRARY->generaltexth->translate("vcmi.lobby.battleOnlyModeLossCondition"));

	const bool isMultiplayer = GAME->server().loadMode == ELoadMode::MULTI;
	if(isMultiplayer)
	{
	...
	const int difficulty = SEL->getCurrentDifficulty();
	const std::array<std::string, 5> difficultyPercent = {"80%", "100%", "130%", "160%", "200%"};
	iconDifficulty->enable();
	iconDifficulty->setSelected(difficulty);
	labelDifficulty->setText(LIBRARY->generaltexth->arraytxt[142 + difficulty]);
	labelDifficultyPercent->setText(difficultyPercent.at(difficulty));
}
...
PvPBox::PvPBox(const Rect & rect)
{
	...
	setHandicapVisible(!GAME->server().battleMode);
}

void PvPBox::setHandicapVisible(bool visible)
{
	buttonHandicap->setEnabled(visible);
}
