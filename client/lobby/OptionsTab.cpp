...
void OptionsTab::SelectionWindow::reopen()
{
	if(type == SelType::HERO && SEL->getStartInfo()->playerInfos.find(color)->second.castle == FactionID::RANDOM)
		...
		close();
		if(GAME->server().isMyColor(color) || GAME->server().loadMode != ELoadMode::MULTI)
...
void OptionsTab::SelectedBox::clickReleased(const Point & cursorPosition)
{
	...
	const bool foreignPlayer = GAME->server().loadMode == ELoadMode::MULTI && !GAME->server().isMyColor(playerSettings.color);
...
{
	if(GAME->server().loadMode == ELoadMode::MULTI && !GAME->server().isMyColor(playerSettings.color))
		return;
	...
	, name(S.name)
{
	...
	const bool foreignMultiplayerPlayer = GAME->server().loadMode == ELoadMode::MULTI && !GAME->server().isMyColor(s->color);
	if(s->isControlledByAI() || foreignMultiplayerPlayer)
...
{
	const bool foreignMultiplayerPlayer = GAME->server().loadMode == ELoadMode::MULTI && !GAME->server().isMyColor(ps.color);
	if(foreignMultiplayerPlayer)
		return;
...
void OptionsTab::PlayerOptionsEntry::hideUnavailableButtons()
{
	...
	const bool foreignPlayer = GAME->server().loadMode == ELoadMode::MULTI && !GAME->server().isMyColor(s->color);
