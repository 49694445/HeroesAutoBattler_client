...
CLevelWindow::CLevelWindow(const CGHeroInstance * hero, PrimarySkill pskill, std::vector<SecondarySkill> & skills, QueryID queryID, int32_t arenaTimeoutSeconds, std::function<void(ui32)> callback)
	...
	skillViewOffset(0)
{
	...
	{
		// Anchor to the panel itself: this stays horizontally centred if the window
		// is scaled, while the bottom margin keeps it clear of the frame and OK button.
...
bool CLevelWindow::setArenaTimeout(QueryID updatedQueryID, int32_t remainingSeconds)
{
	if(queryID != updatedQueryID)
		return false;
...
void CLevelWindow::closeFromServer()
{
	GAME->interface()->showingDialog->setFree();
	CWindowObject::close();
}
	...
	heroToInvite(nullptr)
{
	...
	else if(GAME->interface()->cb->getMapHeader()->battleOnly && GAME->interface()->cb->howManyHeroes(true) >= 3)
	{
		MetaString message;
...
{
	GAME->interface()->showThievesGuildWindow(tavernObj);
}
