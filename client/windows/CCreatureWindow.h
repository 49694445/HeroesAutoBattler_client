...
class CStackWindow : public CWindowObject
{
	...
	class ArenaRecruitSection : public CWindowSection
	{
		CreatureID creature;
		bool battleReadOnly;
		BattleID battleID = BattleID::NONE;
		PlayerColor battlePlayer = PlayerColor::NEUTRAL;
		std::shared_ptr<CLabel> level;
		std::shared_ptr<CLabel> experience;
		std::shared_ptr<CButton> recruit;
		std::vector<std::shared_ptr<CIntObject>> recruitCostWidgets;
		std::string appliedCostSignature;

		/// What refresh() last wrote into the widgets above. refresh() runs on every
		/// draw, and touching a widget costs a redraw request, so it must be able to
		/// tell that nothing has changed and do nothing at all - see refresh().
		std::string appliedLevelText;
		std::string appliedExperienceText;
		std::optional<bool> appliedReadyToRecruit;
		std::optional<bool> appliedBlocked;

		void refresh();
	public:
		ArenaRecruitSection(CStackWindow * owner, int yOffset, bool battleReadOnly);
		void show(Canvas & to) override;
	...
	std::shared_ptr<CWindowSection> arenaRecruitSection;
	...
	bool isArenaRecruitmentWindow() const;
	bool isArenaBattleProgressWindow() const;
...
public:
	...
	void refreshArenaBattleProgress();
	~CStackWindow();
};
