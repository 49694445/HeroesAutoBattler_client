	...
	: CWindowSection(owner, ImagePath::builtin("stackWindow/button-panel"), yOffset)
{
	...
	if(parent->info->dismissInfo && parent->info->dismissInfo->callback)
	{
		...
		auto onClick = [=] ()
		{
			GAME->interface()->showYesNoDialog(getCreatureDismissConfirmationText(parent->info->owner), onDismiss, nullptr);
		};
	...
	if(parent->info->upgradeInfo && !parent->info->commander)
	{
		...
		const bool arenaDeployment = GAME->interface()
			&& GAME->interface()->cb->getMapHeader()->battleOnly
		...
		for(size_t buttonIndex = 0; buttonIndex < buttonsToCreate; buttonIndex++)
		{
			...
			auto upgradeBtn = std::make_shared<CButton>(Point(221 + (int)buttonIndex * 40, 5), AnimationPath::builtin("IVIEWCR.DEF"), LIBRARY->generaltexth->zelp[446], onClick);
			if(arenaDeployment && !GAME->interface()->cb->getResourceAmount().canAfford(totalCost))
...
CStackWindow::ArenaRecruitSection::ArenaRecruitSection(CStackWindow * owner, int yOffset, bool battleReadOnly)
	: CWindowSection(owner, {}, yOffset),
	  creature(owner->info->creature->getId()),
	  battleReadOnly(battleReadOnly)
{
	OBJECT_CONSTRUCTION;
	...
	if(!battleReadOnly)
	{
		...
		for(GameResID resource : LIBRARY->resourceTypeHandler->getAllObjects())
			resourceCount += owner->info->creature->getFullRecruitCost()[resource] > 0;
		pos.h = std::max(36, ((resourceCount + 1) / 2) * 22 + 8);
	}

	level = std::make_shared<CLabel>(30, 18, FONT_SMALL, ETextAlignment::CENTER, Colors::YELLOW);
	experience = std::make_shared<CLabel>(125, 18, FONT_SMALL, ETextAlignment::CENTER, Colors::WHITE);
	// A label normally repaints itself the instant its text is set. refresh() runs mid-draw
	// and changes several widgets at once, so letting any one of them paint before the rest
	// have been updated puts a half-updated frame on screen. refresh() asks for the repaint
	// itself, once, after everything is in place.
	level->setAutoRedraw(false);
	experience->setAutoRedraw(false);
	if(battleReadOnly)
	{
		if(owner->info->stack && owner->info->stack->getBattle())
		{
			battleID = owner->info->stack->getBattle()->battleID;
			battlePlayer = owner->info->stack->unitOwner();
		}
	}
	else
	{
		recruit = std::make_shared<CButton>(Point(190, 2), AnimationPath::builtin("IBY6432.DEF"), CButton::tooltip(), [this]()
		{
			if(!GAME->interface() || !GAME->server().client)
				return;

			ArenaRecruitAction request;
			request.action = ArenaRecruitAction::EAction::RECRUIT;
...
/// Brings the widgets in line with the player's current recruitment state.
///
/// Called from show() and showAll(), so this runs inside a draw pass, and that makes
/// every widget call here dangerous: CLabel::setText redraws unconditionally, without
/// comparing against the text already there, and a CLabel redraws through its parent.
/// The parent is this section, whose redraw() ends in showAll() - which calls refresh()
/// again. Writing a label on every draw therefore recurses until the stack runs out,
/// which kills the process outright: no exception, no crash dump, nothing in the log
/// past the redraw that started it.
///
/// So nothing may be written unless it actually differs from what is already displayed.
/// Once the values settle, this function must touch nothing at all.
void CStackWindow::ArenaRecruitSection::refresh()
{
	if(!GAME->interface())
		return;
	if(battleReadOnly)
	{
		const auto progress = GAME->interface()->getArenaBattleCreatureProgress(battleID, battlePlayer, creature);
		const std::string levelText = progress ? "Lv" + std::to_string(progress->level) : "Lv--";
		const std::string experienceText = progress ? std::to_string(progress->currentExperience) + "/" + std::to_string(progress->maximumExperience) : "--/--";
		bool changed = false;
		if(appliedLevelText != levelText)
		{
			appliedLevelText = levelText;
			level->setText(levelText);
			changed = true;
		}
		if(appliedExperienceText != experienceText)
		{
			appliedExperienceText = experienceText;
	...
	const auto levelIt = playerState->arenaRecruitLevel.find(creature);
	const int32_t recruitLevel = levelIt == playerState->arenaRecruitLevel.end() ? 1 : levelIt->second;

	const auto experienceIt = playerState->arenaCreatureExperience.find(creature);
	const auto thresholdIt = playerState->arenaRecruitThreshold.find(creature);
	const int64_t experienceValue = experienceIt == playerState->arenaCreatureExperience.end() ? 0 : experienceIt->second;
	const bool hasThreshold = thresholdIt != playerState->arenaRecruitThreshold.end();
	const bool readyToRecruit = hasThreshold && experienceValue >= thresholdIt->second;

	const std::string levelText = "Lv" + std::to_string(recruitLevel);
	// Only shown while still saving up; once the button appears it takes this spot.
	const std::string experienceText = readyToRecruit
		? std::string()
		: std::to_string(experienceValue) + "/" + (hasThreshold ? std::to_string(thresholdIt->second) : "--");

	const auto costIt = playerState->arenaRecruitCost.find(creature);
	const bool unlocked = playerState->arenaUnlockedCreatures.contains(creature);
	const bool affordable = costIt != playerState->arenaRecruitCost.end() && playerState->resources.canAfford(costIt->second);
	const bool blocked = !unlocked || !affordable;
	std::string costSignature = costIt == playerState->arenaRecruitCost.end() ? "unknown" : "known";
	std::vector<GameResID> pricedResources;
	if(costIt != playerState->arenaRecruitCost.end())
	{
		const auto addResource = [&](GameResID resource)
		{
			const auto required = costIt->second[resource];
			if(required <= 0)
				return;
			pricedResources.push_back(resource);
			costSignature += ":" + std::to_string(resource.getNum()) + ":" + std::to_string(required)
				+ (playerState->resources[resource] < required ? ":short" : ":enough");
		};
		addResource(GameResID::GOLD);
		for(GameResID resource : LIBRARY->resourceTypeHandler->getAllObjects())
			if(resource != GameResID::GOLD)
				addResource(resource);
	}

	bool changed = false;
	if(appliedCostSignature != costSignature)
	{
		// Cache before creating children: refresh also runs during redraw.
		appliedCostSignature = costSignature;
		recruitCostWidgets.clear();
		OBJECT_CONSTRUCTION_TARGETED(this);
		if(pricedResources.empty())
		{
			auto text = std::make_shared<CLabel>(268, 18, FONT_SMALL, ETextAlignment::CENTERLEFT,
				Colors::WHITE, costIt == playerState->arenaRecruitCost.end() ? LIBRARY->generaltexth->translate("vcmi.arena.recruit.costPending") : LIBRARY->generaltexth->translate("vcmi.arena.recruit.free"));
			text->setAutoRedraw(false);
			recruitCostWidgets.push_back(text);
		}
		for(size_t i = 0; i < pricedResources.size(); ++i)
		{
			const auto resource = pricedResources[i];
			const int x = 265 + static_cast<int>(i % 2) * 81;
			const int y = 7 + static_cast<int>(i / 2) * 22;
			auto icon = std::make_shared<CAnimImage>(AnimationPath::builtin("SMALRES"),
				LIBRARY->resourceTypeHandler->getById(resource)->getIconIndex(), 0, x, y);
			const auto required = costIt->second[resource];
			auto amount = std::make_shared<CLabel>(x + icon->pos.w + 3, y + icon->pos.h / 2,
				FONT_SMALL, ETextAlignment::CENTERLEFT,
				playerState->resources[resource] < required ? Colors::RED : Colors::WHITE,
				std::to_string(required));
			amount->setAutoRedraw(false);
			recruitCostWidgets.push_back(icon);
			recruitCostWidgets.push_back(amount);
		}
		changed = true;
	}
	...
	if(!readyToRecruit && appliedExperienceText != experienceText)
	{
		appliedExperienceText = experienceText;
		experience->setText(experienceText);
		changed = true;
	...
	// Blocking before enabling, so the button is never briefly shown in the wrong state.
	if(readyToRecruit && appliedBlocked != blocked)
	{
		appliedBlocked = blocked;
		recruit->block(blocked);
		changed = true;
	}

	if(appliedReadyToRecruit != readyToRecruit)
	{
		appliedReadyToRecruit = readyToRecruit;
		changed = true;
		...
		else
		{
			recruit->disable();
			experience->enable();
			// The button will need blocking again from scratch next time it appears.
			appliedBlocked.reset();
		}
	}

	// Everything is in place now, so one repaint shows a consistent state. Widgets that
	// paint on their own were left out of that job on purpose - see the constructor.
	// The repaint calls back into refresh(), which finds nothing left to do and stops.
	if(changed)
		redraw();
}

void CStackWindow::ArenaRecruitSection::show(Canvas & to)
{
	refresh();
	CIntObject::show(to);
}

void CStackWindow::ArenaRecruitSection::showAll(Canvas & to)
{
	refresh();
...
void CStackWindow::initSections()
{
	...
	if(isArenaBattleProgressWindow())
	{
		arenaRecruitSection = std::make_shared<ArenaRecruitSection>(this, pos.h, true);
		pos.h += arenaRecruitSection->pos.h;
	}
	else if(isArenaRecruitmentWindow())
	{
		arenaRecruitSection = std::make_shared<ArenaRecruitSection>(this, pos.h, false);
		pos.h += arenaRecruitSection->pos.h;
	}
...
bool CStackWindow::isArenaRecruitmentWindow() const
{
	return GAME->interface()
		&& info->owner
		&& info->owner->tempOwner == GAME->interface()->playerID
		&& GAME->interface()->cb->getMapHeader()->battleOnly;
}

bool CStackWindow::isArenaBattleProgressWindow() const
{
	return GAME->interface()
		&& info->stack
		&& info->stack->getBattle()
		&& info->stack->unitOwner().isValidPlayer()
		&& GAME->interface()->cb->getMapHeader()->battleOnly;
}

void CStackWindow::refreshArenaBattleProgress()
{
	if(arenaRecruitSection && isArenaBattleProgressWindow())
		arenaRecruitSection->redraw();
}
