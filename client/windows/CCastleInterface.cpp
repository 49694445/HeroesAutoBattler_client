...
{
	if(GAME->interface()->isArenaTownVisit(town))
		return;
	bool allow = true;
...
void CCastleBuildings::enterDwelling(int level)
{
	...
	if(GAME->interface()->isArenaTownVisit(town))
	{
		BuildingID building = BuildingID(BuildingID::getDwellingFromLevel(level, 0));
		if(level == town->getTown()->creatures.size())
			building = town->getTown()->getSpecialBuilding(BuildingSubID::PORTAL_OF_SUMMONING)->bid;
		else
		{
			BuildingID builtDwelling = building;
			for(; town->getTown()->buildings.count(building); BuildingID::advanceDwelling(building))
				if(town->hasBuilt(building))
					builtDwelling = building;
			building = builtDwelling;
		}

		if(GAME->interface()->requestArenaTownSummon(town, building, town->creatures[level].second.back()))
			return;
	}
...
const CBuilding * CCreaInfo::getArenaDwelling() const
{
	...
	BuildingID building = BuildingID::getDwellingFromLevel(level, 0);
	const CBuilding * result = nullptr;
	for(; town->getTown()->buildings.count(building); BuildingID::advanceDwelling(building))
		if(town->hasBuilt(building))
			result = town->getTown()->buildings.at(building).get();
	return result;
}

void CCreaInfo::refreshArenaTownSummon()
{
	arenaSummonState.reset();
	if(label && GAME->interface()->cb->getMapHeader()->battleOnly)
		label->setText("-");
	const auto * building = getArenaDwelling();
	if(!building || level < 0 || town->creatures[level].second.empty())
		return;

	const auto state = GAME->interface()->getArenaTownSummonState(town, building->bid, creature);
	if(!state)
		return;

	// Arena quantities come from the same authoritative offer as the summon
	// action, including recruitment level and the server's growth rules.
	// Keep the quantity visible even when money/allowance/army blocks summoning.
	if(label)
		label->setText(state->amount > 0 ? std::to_string(state->amount) : "-");

	std::string image;
	switch(state->status)
	{
		case EArenaTownSummonStatus::AVAILABLE:
			image = "arenaSummonAvailable.png";
			break;
		case EArenaTownSummonStatus::UNAVAILABLE:
			image = "arenaSummonUnavailable.png";
			break;
		case EArenaTownSummonStatus::ALREADY_IN_ARMY:
			image = "arenaSummonAlreadyInArmy.png";
			break;
	}

	const int overlayX = pos.w == 32 ? 0 : 8;
	{
		OBJECT_CONSTRUCTION_TARGETED(this);
		arenaSummonState = std::make_shared<CPicture>(ImagePath::builtin(image), overlayX, 26);
		arenaSummonState->scaleTo(Point(32, 6));
		arenaSummonState->removeUsedEvents(LCLICK | SHOW_POPUP);
	}
}
...
{
	MetaString message;
	message.appendTextID(GAME->interface()->cb->getMapHeader()->battleOnly
		? "vcmi.arena.town.summonQuantity.hover" : "core.genrltxt.588");
...
{
	const auto * building = getArenaDwelling();
	if(building && GAME->interface()->requestArenaTownSummon(town, building->bid, creature))
	{
		refreshArenaTownSummon();
		return;
	}
	...
	town(Town)
{
	...
	garr->setTransfersEnabled(!GAME->interface()->isArenaTownVisit(town));
	...
	{
		OBJECT_CONSTRUCTION_TARGETED(this);
		arenaSummonsIcon = std::make_shared<CPicture>(ImagePath::builtin("arenaSummonsRemaining"), 15, 399);
		arenaSummonsIcon->scaleTo(Point(17, 24));
		arenaSummonsCount = std::make_shared<CLabel>(38, 399, FONT_BIG, ETextAlignment::TOPLEFT, Colors::WHITE);
		refreshArenaSummonsRemaining();
	}
...
{
	if(GAME->interface()->requestArenaTownLeave(town))
		return;
...
void CCastleInterface::closeAfterArenaLeave()
{
	CWindowObject::close();
}
...
void CCastleInterface::townChange()
{
	...
	if ( dest == town )
		return;
	if(GAME->interface()->requestArenaTownSwitch(dest))
	{
	...
	building(Building)
{
	...
	if(building->bid == BuildingID::FORT)
		if(const auto dynamicCost = GAME->interface()->getArenaTownFortCost(town))
			if(state == EBuildingState::ALLOWED || state == EBuildingState::NO_RESOURCES)
				state = dynamicCost->canBeAfforded(GAME->interface()->cb->getPlayerState(town->tempOwner)->resources)
					? EBuildingState::ALLOWED : EBuildingState::NO_RESOURCES;
	if(GAME->interface()->isArenaTownBuildGateLocked(town, building->bid))
...
bool CHallInterface::isForTown(ObjectInstanceID townID) const
{
	return town && town->id == townID;
}

void CHallInterface::refreshArenaBuildGate()
{
	boxes.clear();
	rebuildBuildingBoxes();
	redraw();
...
void CHallInterface::rebuildBuildingBoxes()
{
...
CBuildWindow::CBuildWindow(const CGTownInstance *Town, const CBuilding * Building, EBuildingState state, bool rightClick):
	...
	building(Building),
	buildingState(state),
	baseBuildingState(GAME->interface()->cb->canBuildStructure(Town, Building->bid))
{
	OBJECT_CONSTRUCTION;
	TResources displayedBuildingCost = building->resources;
	if(building->bid == BuildingID::FORT)
		if(const auto dynamicCost = GAME->interface()->getArenaTownFortCost(town))
		{
			displayedBuildingCost = *dynamicCost;
			if(buildingState == EBuildingState::ALLOWED || buildingState == EBuildingState::NO_RESOURCES)
			{
				buildingState = displayedBuildingCost.canBeAfforded(GAME->interface()->cb->getPlayerState(town->tempOwner)->resources)
					? EBuildingState::ALLOWED : EBuildingState::NO_RESOURCES;
				state = buildingState;
			}
		}
	...
	{
		if(displayedBuildingCost[i])
		{
			MetaString message;
			int resourceAmount = GAME->interface()->cb->getResourceAmount(i);
			bool canAfford = resourceAmount >= displayedBuildingCost[i];
			...
			message.replaceNumber(displayedBuildingCost[i]);
	...
	refreshArenaBuildGate();
}

void CBuildWindow::activate()
{
	CStatusbarWindow::activate();
	if(!arenaQuoteRequested && canUseArenaBuildLevel() && ENGINE->windows().isTopWindow(this))
		requestArenaQuotes();
}

void CBuildWindow::buyFunc()
{
	if(arenaSubmitPending)
		return;
	if(!arenaQuotes.empty())
	{
		const auto level = arenaLevelSlider->getValue() + 1;
		if(level < 1 || level > arenaQuotes.size())
			return;
		arenaSubmitPending = true;
		buy->block(true);
		arenaLevelSlider->block(true);
		ArenaBuildDwellingWithLevel request;
		request.town = town->id;
		request.building = building->bid;
		request.creature = arenaQuoteCreature;
		request.level = level;
		request.expectedRevision = arenaQuoteRevision;
		GAME->server().client->sendRequest(request, GAME->interface()->playerID, false);
		return;
	}
...
bool CBuildWindow::canUseArenaBuildLevel() const
{
	const bool dynamicFort = building->bid == BuildingID::FORT
		&& (buildingState == EBuildingState::ALLOWED || buildingState == EBuildingState::NO_RESOURCES);
	return GAME->interface()->cb->getMapHeader()->battleOnly
		&& buy
		&& ((building->bid.isDwelling() && buildingState == EBuildingState::ALLOWED) || dynamicFort)
		&& GAME->interface()->playerID == town->tempOwner
		&& GAME->interface()->makingTurn;
}

std::string CBuildWindow::getArenaBuildGateText()
{
	auto required = town->getTown()->buildings.find(arenaBuildGateRequired);
	if(required == town->getTown()->buildings.end())
	{
		logGlobal->warn("[ARENA] missing required build gate building town=%d building=%d required=%d", town->id.getNum(), building->bid.getNum(), arenaBuildGateRequired.getNum());
		return getTextForState(EBuildingState::PREREQUIRES);
	}
	std::string message = LIBRARY->generaltexth->translate("vcmi.townHall.missingBase");
	return boost::str(boost::format(message) % required->second->getNameTranslated());
}

bool CBuildWindow::isForTown(ObjectInstanceID townID) const
{
	return town && town->id == townID;
}

void CBuildWindow::refreshArenaBuildGate()
{
	baseBuildingState = GAME->interface()->cb->canBuildStructure(town, building->bid);
	if(building->bid == BuildingID::FORT)
		if(const auto dynamicCost = GAME->interface()->getArenaTownFortCost(town))
		{
			rebuildCost(*dynamicCost);
			if(baseBuildingState == EBuildingState::ALLOWED || baseBuildingState == EBuildingState::NO_RESOURCES)
				baseBuildingState = dynamicCost->canBeAfforded(GAME->interface()->cb->getPlayerState(town->tempOwner)->resources)
					? EBuildingState::ALLOWED : EBuildingState::NO_RESOURCES;
		}
	BuildingID requiredBuilding;
	if(GAME->interface()->isArenaTownBuildGateLocked(town, building->bid, &requiredBuilding))
	{
		arenaBuildGateRequired = requiredBuilding;
		buildingState = EBuildingState::PREREQUIRES;
		clearArenaQuoteControls();
		arenaQuoteRequested = false;
		arenaQuotePending = false;
		arenaSubmitPending = false;
		stateText->setText(getArenaBuildGateText());
		if(buy)
			buy->block(true);
		redraw();
		return;
	...
	arenaBuildGateRequired = BuildingID::NONE;
	buildingState = baseBuildingState;
	stateText->setText(getTextForState(buildingState));
	if(buy)
		buy->block(buildingState != EBuildingState::ALLOWED || GAME->interface()->playerID != town->tempOwner || !GAME->interface()->makingTurn);
	if(!arenaQuoteRequested && canUseArenaBuildLevel() && ENGINE->windows().isTopWindow(this))
		requestArenaQuotes();
	redraw();
}

void CBuildWindow::requestArenaQuotes()
{
	if(!canUseArenaBuildLevel())
		return;
	arenaQuoteRequested = true;
	arenaQuotePending = true;
	buy->block(true);
	stateText->setText(LIBRARY->generaltexth->translate("vcmi.arena.recruit.quoteLoading"));
	ArenaBuildLevelQuoteRequest request;
	request.town = town->id;
	request.building = building->bid;
	logGlobal->debug("[ARENA] sending build-level quote request town=%d building=%d", request.town.getNum(), request.building.getNum());
	GAME->server().client->sendRequest(request, GAME->interface()->playerID, false);
}

void CBuildWindow::clearArenaQuoteControls()
{
	arenaQuotes.clear();
	arenaQuoteRevision = 0;
	arenaQuoteCreature = CreatureID::NONE;
	arenaLevelSlider.reset();
	arenaLevelText.reset();
	arenaAmountText.reset();
	arenaThresholdText.reset();
	arenaSummonCostItems.clear();
	arenaQuotePanelBorder.reset();
	arenaSummonCostText.reset();
	arenaQuotePanel.reset();
}

void CBuildWindow::rebuildCost(const TResources & resources)
{
	std::vector<std::shared_ptr<CComponent>> components;
	for(GameResID resource : LIBRARY->resourceTypeHandler->getAllObjects())
	{
		if(!resources[resource])
			continue;
		const auto owned = GAME->interface()->cb->getResourceAmount(resource);
		MetaString text;
		if(owned < resources[resource] && settings["general"]["enableUiEnhancements"].Bool())
		{
			text.appendRawString("{H3Red|%d}/%d");
			text.replaceNumber(owned);
		}
		else
			text.appendRawString("%d");
		text.replaceNumber(resources[resource]);
		components.push_back(std::make_shared<CComponent>(ComponentType::RESOURCE, resource, text.toString(), CComponent::small));
	}
	cost.reset();
	{
		OBJECT_CONSTRUCTION_TARGETED(this);
		cost = std::make_shared<CComponentBox>(components, Rect(25, arenaQuotes.empty() ? 330 : 365, pos.w - 50, 65));
	}
}

void CCastleInterface::refreshArenaTownSummons()
{
	for(auto & creatureInfoBox : creainfo)
		creatureInfoBox->refreshArenaTownSummon();
	redraw();
}

void CCastleInterface::refreshArenaSummonsRemaining()
{
	if(!arenaSummonsCount || !GAME->interface() || !GAME->interface()->isArenaTownVisit(town))
		return;

	const auto * playerState = GAME->interface()->cb->getPlayerState(GAME->interface()->playerID);
	arenaSummonsCount->setText(playerState ? std::to_string(playerState->arenaSummonsRemaining) : std::string());
	redraw();
}

void CBuildWindow::setArenaError(const std::string & reason)
{
	clearArenaQuoteControls();
	arenaQuotePending = false;
	arenaSubmitPending = false;
	stateText->setText(reason.empty() ? LIBRARY->generaltexth->translate("vcmi.arena.recruit.quoteInvalid") : reason);
	buy->block(true);
	redraw();
}

void CBuildWindow::refreshArenaQuote(int level)
{
	if(level < 1 || level > arenaQuotes.size())
	{
		setArenaError(LIBRARY->generaltexth->translate("vcmi.arena.recruit.quoteInvalid"));
		return;
	}
	const auto & quote = arenaQuotes[level - 1];
	arenaLevelText->setText(LIBRARY->generaltexth->translate("vcmi.arena.recruit.levelPrefix") + std::to_string(quote.level));
	arenaAmountText->setText(LIBRARY->generaltexth->translate("vcmi.arena.recruit.amountPrefix") + std::to_string(quote.amount));
	arenaThresholdText->setText(LIBRARY->generaltexth->translate("vcmi.arena.recruit.experiencePrefix") + std::to_string(quote.threshold));
	arenaSummonCostItems.clear();
	arenaSummonCostText->setText(LIBRARY->generaltexth->translate("vcmi.arena.recruit.summonCostTitle"));
	std::vector<GameResID> resources;
	if(quote.summonCostValid)
	{
		if(quote.summonCost[GameResID::GOLD])
			resources.push_back(GameResID::GOLD);
		for(GameResID resource : LIBRARY->resourceTypeHandler->getAllObjects())
			if(resource != GameResID::GOLD && quote.summonCost[resource])
				resources.push_back(resource);
	}
	{
		OBJECT_CONSTRUCTION_TARGETED(this);
		if(resources.empty())
			arenaSummonCostItems.push_back(std::make_shared<CLabel>(197, 309, FONT_SMALL,
				ETextAlignment::CENTER, Colors::WHITE, quote.summonCostValid ? LIBRARY->generaltexth->translate("vcmi.arena.recruit.free") : LIBRARY->generaltexth->translate("vcmi.arena.recruit.quoteUnavailable")));
		for(size_t first = 0; first < resources.size(); first += 3)
		{
			std::vector<std::pair<std::shared_ptr<CAnimImage>, std::shared_ptr<CLabel>>> row;
			int width = 0;
			for(size_t i = first; i < std::min(first + 3, resources.size()); ++i)
			{
				auto icon = std::make_shared<CAnimImage>(AnimationPath::builtin("SMALRES"),
					LIBRARY->resourceTypeHandler->getById(resources[i])->getIconIndex(), 0, 0, 0);
				auto amount = std::make_shared<CLabel>(0, 0, FONT_SMALL, ETextAlignment::TOPLEFT,
					Colors::WHITE, std::to_string(quote.summonCost[resources[i]]));
				width += icon->pos.w + 4 + amount->pos.w + 16;
				row.emplace_back(icon, amount);
			}
			int x = (395 - (width - 16)) / 2;
			const int y = 300 + static_cast<int>(first / 3) * 20;
			for(auto & [icon, amount] : row)
			{
				icon->moveBy(Point(x, y));
				amount->moveBy(Point(x + icon->pos.w + 4, y + (icon->pos.h - amount->pos.h) / 2));
				x += icon->pos.w + 4 + amount->pos.w + 16;
				arenaSummonCostItems.push_back(icon);
				arenaSummonCostItems.push_back(amount);
			}
		}
	}
	...
	for(GameResID resource : LIBRARY->resourceTypeHandler->getAllObjects())
	{
		if(!quote.totalCost[resource])
			continue;
		const auto owned = GAME->interface()->cb->getResourceAmount(resource);
		canAfford &= owned >= quote.totalCost[resource];
	}
	rebuildCost(quote.totalCost);
	buy->block(arenaSubmitPending || !canAfford);
	redraw();
}

void CBuildWindow::applyArenaBuildLevelState(const SetArenaBuildLevelState & state)
{
	if(state.town != town->id || state.building != building->bid)
		return;
	logGlobal->debug("[ARENA] matched build-level state town=%d building=%d revision=%u status=%d", state.town.getNum(), state.building.getNum(), state.revision, static_cast<int>(state.status));
	if(state.status == EArenaBuildLevelStatus::LOCKED)
	{
		clearArenaQuoteControls();
		arenaQuotePending = false;
		arenaSubmitPending = false;
		rebuildCost(building->resources);
		stateText->setText(getTextForState(buildingState));
		buy->block(buildingState != EBuildingState::ALLOWED || GAME->interface()->playerID != town->tempOwner || !GAME->interface()->makingTurn);
		redraw();
		return;
	}
	if(state.status == EArenaBuildLevelStatus::REJECTED)
	{
		if(!arenaQuotePending && !arenaSubmitPending)
			return;
		setArenaError(state.reason);
		return;
	}
	if(state.status == EArenaBuildLevelStatus::SUCCEEDED)
	{
		if(!arenaSubmitPending || state.revision <= arenaQuoteRevision)
			return;
		ENGINE->windows().popWindows(2);
		return;
	}
	if(state.status == EArenaBuildLevelStatus::QUOTES && building->bid == BuildingID::FORT && state.quotes.empty())
	{
		if(!arenaQuotePending || state.revision <= arenaQuoteRevision)
			return;
		arenaQuoteRevision = state.revision;
		arenaQuotePending = false;
		rebuildCost(state.buildingCost);
		bool canAfford = true;
		for(GameResID resource : LIBRARY->resourceTypeHandler->getAllObjects())
			canAfford &= GAME->interface()->cb->getResourceAmount(resource) >= state.buildingCost[resource];
		stateText->setText("");
		buy->block(!canAfford);
		redraw();
		return;
	}
	if(state.status == EArenaBuildLevelStatus::QUOTES && state.revision <= arenaQuoteRevision)
		return;
	if(state.status != EArenaBuildLevelStatus::QUOTES || !arenaQuotePending || state.revision == 0 || state.maxLevel < 1 || state.quotes.size() != state.maxLevel)
	{
		if(state.status == EArenaBuildLevelStatus::QUOTES)
			setArenaError(LIBRARY->generaltexth->translate("vcmi.arena.recruit.quoteInvalid"));
		return;
	}
	for(size_t i = 0; i < state.quotes.size(); ++i)
		if(state.quotes[i].level != i + 1)
		{
			setArenaError(LIBRARY->generaltexth->translate("vcmi.arena.recruit.quoteInvalid"));
			return;
		}
	arenaQuotes = state.quotes;
	arenaQuoteRevision = state.revision;
	arenaQuoteCreature = state.creature;
	arenaQuotePending = false;
	stateText->setText("");
	{
		OBJECT_CONSTRUCTION_TARGETED(this);
		// Extend the dark panel for the server-quoted summon cost below the slider.
		arenaQuotePanel = std::make_shared<CFilledTexture>(ImagePath::builtin("TPUBUILD"),
			Rect(25, 208, 345, 153), Rect(34, 224, 320, 40));
		arenaQuotePanelBorder = std::make_shared<TransparentFilledRectangle>(Rect(25, 208, 345, 153),
			ColorRGBA(0, 0, 0, 0), Colors::METALLIC_GOLD);
		arenaSummonCostText = std::make_shared<CTextBox>("", Rect(33, 281, 329, 16),
			0, FONT_SMALL, ETextAlignment::CENTER, Colors::WHITE);
		arenaLevelText = std::make_shared<CLabel>(35, 220, FONT_SMALL, ETextAlignment::TOPLEFT, Colors::WHITE);
		arenaAmountText = std::make_shared<CLabel>(197, 220, FONT_SMALL, ETextAlignment::CENTER, Colors::WHITE);
		arenaThresholdText = std::make_shared<CLabel>(360, 220, FONT_SMALL, ETextAlignment::TOPRIGHT, Colors::WHITE);
		arenaLevelText->setAutoRedraw(false);
		arenaAmountText->setAutoRedraw(false);
		arenaThresholdText->setAutoRedraw(false);
		arenaLevelSlider = std::make_shared<CSlider>(Point(55, 252), 285, [this](int index) { refreshArenaQuote(index + 1); }, 1, state.maxLevel, 0, Orientation::HORIZONTAL);
	}
	refreshArenaQuote(1);
}
...
bool CFortScreen::isForTown(ObjectInstanceID town) const
{
	return townId == town;
}

void CFortScreen::refreshArenaTownSummons()
{
	for(auto & area : recAreas)
		area->refreshArenaTownSummon();
	redraw();
}
...
void CFortScreen::RecruitArea::refreshArenaTownSummon()
{
	arenaSummonState.reset();
	const auto * building = getMyBuilding();
	const auto * creature = getMyCreature();
	if(!building || !creature || !town->hasBuilt(building->bid))
		return;

	const auto state = GAME->interface()->getArenaTownSummonState(town, building->bid, creature->getId());
	if(!state)
		return;
	hoverText = boost::str(boost::format(LIBRARY->generaltexth->tcommands[21]) % creature->getNamePluralTranslated());
	if(state->pending)
	...
	{
		OBJECT_CONSTRUCTION_TARGETED(this);
		arenaSummonState = std::make_shared<CPicture>(ImagePath::builtin(image), 172, 35);
	}
}
...
void CFortScreen::RecruitArea::clickPressed(const Point & cursorPosition)
{
	const auto * building = getMyBuilding();
	const auto * creature = getMyCreature();
	if(building && creature && GAME->interface()->requestArenaTownSummon(town, building->bid, creature->getId()))
	{
		refreshArenaTownSummon();
		return;
	}
...
{
	// Arena deployment keeps each army face-down.  Spell research is a separate
	// town mutation and is intentionally unavailable there; leave the scroll
	// readable (including right-click help) but make its left click inert.
