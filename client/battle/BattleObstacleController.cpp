...
void BattleObstacleController::obstaclePlaced(const std::vector<std::shared_ptr<const CObstacleInstance>> & obstacles)
{
	for(const auto & oi : obstacles)
	{
		// Siege controller owns moat visuals. The automatic moat-placement pack
		// has no generic appear animation, so do not route it through the normal
		// obstacle animation path.
		if(oi->obstacleType == CObstacleInstance::MOAT)
			continue;
