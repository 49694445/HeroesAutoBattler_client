...
{
	const BattleID expectedBattleID = battleID;
	auto onIntroPlayed = [expectedBattleID]()
	{
		// Resolve the current interface instead of retaining a raw pointer: the sound
		// callback can be dispatched after an asynchronously resolved Arena battle.
		auto currentBattleInterface = GAME->interface()->battleInt;
		if(currentBattleInterface && currentBattleInterface->getBattleID() == expectedBattleID)
			currentBattleInterface->onIntroSoundPlayed();
	};

	auto bfieldType = getBattle()->battleGetBattlefieldType();
	const auto & battlefieldSound = bfieldType.getInfo()->musicFilename;
	const bool arenaBattle = curInt && curInt->cb->getMapHeader()->battleOnly;
	...
	if (!arenaBattle && !battlefieldSound.empty())
...
void BattleInterface::onIntroSoundPlayed()
{
	...
	// Arena result handling removes the player's battle callback before all
	// already queued audio events are dispatched. Keep the callback alive for
	// this method, or quietly ignore a completion event for an ended battle.
	std::shared_ptr<CPlayerBattleCallback> battle;
	try
	{
		battle = curInt->cb->getBattle(battleID);
	}
	...
	auto bfieldType = battle->battleGetBattlefieldType();
	const auto & battlefieldMusic = bfieldType.getInfo()->musicFilename;
	const bool arenaBattle = curInt && curInt->cb->getMapHeader()->battleOnly;

	if(arenaBattle)
	{
		const bool siegeBattle = battle->battleGetDefendedTown() != nullptr;
		if(siegeBattle)
		{
			ENGINE->music().playMusic(AudioPath::builtin("Music/Combat04"), true, true);
		}
		else
		{
			const std::array<AudioPath, 3> arenaBattleMusic =
			{
				AudioPath::builtin("Music/Combat01"),
				AudioPath::builtin("Music/Combat02"),
				AudioPath::builtin("Music/Combat03")
			};
			ENGINE->music().playMusic(*RandomGeneratorUtil::nextItem(arenaBattleMusic, CRandomGenerator::getDefault()), true, true);
		}
	}
	else if (!battlefieldMusic.empty())
...
{
	if(battleIntroSoundChannel != -1)
		ENGINE->sound().resetCallback(battleIntroSoundChannel);
	if(windowObject)
		windowObject->destroyEnemyHeroWindow();
...
void BattleInterface::battleFinished(const BattleResult& br, QueryID queryID)
{
	...
	const bool isArena = curInt->cb->getMapHeader()->battleOnly;
	if(isArena)
		windowObject->quiesceForBattleResult();
	if(!isArena)
	...
	const int32_t arenaTimeout = isArena ? curInt->getArenaBattleResultTimeout(queryID) : -1;
	auto wnd = std::make_shared<BattleResultWindow>(br, *(this->curInt), false, queryID, arenaTimeout);
	wnd->resultCallback = [this, queryID](ui32 selection)
	{
		curInt->cb->selectionMade(selection, queryID);
	};
	if(isArena && ENGINE->windows().isTopWindow(windowObject))
	{
		// BattleResultWindow has synchronously copied everything it needs from the
		// battle. Remove the renderer before BattleEnded can delete battle state.
		ENGINE->windows().popWindows(1);
		logGlobal->info("[ARENA] removed battle window before showing result: battle=%d", getBattleID().getNum());
	}
	else if(isArena)
		logGlobal->warn("[ARENA] battle window was not topmost before result: battle=%d; renderer remains quiesced", getBattleID().getNum());
...
void BattleInterface::showEnemyHeroWindow(const CGHeroInstance * hero) const
{
	if(!hero || !curInt || !curInt->cb->getMapHeader()->battleOnly)
		return;

	if(hero != attackingHeroInstance && hero != defendingHeroInstance)
		return;

	if(getBattle()->playerToSide(curInt->playerID) == BattleSide::NONE || hero->tempOwner == curInt->playerID)
		return;

	windowObject->showEnemyHeroWindow(hero);
}
