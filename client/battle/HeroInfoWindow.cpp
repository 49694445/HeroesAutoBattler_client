...
HeroInfoBasicPanel::HeroInfoBasicPanel(const InfoAboutHero & hero, const Point * position, bool initializeBackground, std::function<void()> portraitClick)
	: CIntObject(portraitClick ? LCLICK : 0)
	, onPortraitClick(std::move(portraitClick))
{
	OBJECT_CONSTRUCTION;
	if(position != nullptr)
		moveTo(*position);
...
Rect HeroInfoBasicPanel::portraitArea() const
{
	return Rect(pos.x + 10, pos.y + 6, 58, 64);
}

bool HeroInfoBasicPanel::receiveEvent(const Point & cursorPosition, int eventType) const
{
	return eventType == LCLICK && onPortraitClick && portraitArea().isInside(cursorPosition);
}

void HeroInfoBasicPanel::clickPressed(const Point & cursorPosition)
{
	if(onPortraitClick && portraitArea().isInside(cursorPosition))
		onPortraitClick();
}
