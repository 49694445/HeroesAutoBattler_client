...
class BattleWindow : public InterfaceObjectConfigurable
{
	...
	std::shared_ptr<CHeroWindow> enemyHeroWindow;
	bool enemyHeroWindowVisible = false;
	...
	bool arenaExitButtonsHidden = false;
	bool quiescedForBattleResult = false;
	void hideArenaExitButtons();
...
public:
	...
	void close();
	/// Stops all battle UI event receivers while the asynchronous result window is shown.
	void quiesceForBattleResult();
	/// Shows the enemy hero in a non-blocking, read-only child panel.
	void showEnemyHeroWindow(const CGHeroInstance * hero);
	void closeEnemyHeroWindow();
	void destroyEnemyHeroWindow();
