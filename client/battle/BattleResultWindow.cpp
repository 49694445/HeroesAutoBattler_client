...
BattleResultWindow::BattleResultWindow(const BattleResult & br, CPlayerInterface & _owner, bool allowReplay, QueryID arenaQueryID, int32_t arenaTimeoutSeconds)
	: owner(_owner), arenaQueryID(arenaQueryID)
{
	OBJECT_CONSTRUCTION;
	...
	if(owner.cb->getMapHeader()->battleOnly)
	{
		const auto healthDamageText = [](int32_t damage)
		{
			return damage > 0 ? "-" + std::to_string(damage) : "0";
		};
		labels.push_back(std::make_shared<CLabel>(59, 148, FONT_SMALL, ETextAlignment::CENTER, Colors::WHITE, healthDamageText(br.arenaHealthDamage[BattleSide::ATTACKER])));
		labels.push_back(std::make_shared<CLabel>(412, 148, FONT_SMALL, ETextAlignment::CENTER, Colors::WHITE, healthDamageText(br.arenaHealthDamage[BattleSide::DEFENDER])));
		if(arenaTimeoutSeconds >= 0)
			arenaTimeoutLabel = std::make_shared<CLabel>(pos.w / 2, pos.h - 14, FONT_SMALL, ETextAlignment::BOTTOMCENTER, Colors::WHITE, std::to_string(arenaTimeoutSeconds));
	}
...
bool BattleResultWindow::setArenaTimeout(QueryID queryID, int32_t remainingSeconds)
{
	if(arenaQueryID != queryID)
		return false;
	if(!arenaTimeoutLabel)
		arenaTimeoutLabel = std::make_shared<CLabel>(pos.w / 2, pos.h - 14, FONT_SMALL, ETextAlignment::BOTTOMCENTER, Colors::WHITE, std::to_string(remainingSeconds));
	else
		arenaTimeoutLabel->setText(std::to_string(remainingSeconds));
	redraw();
	return true;
}

void BattleResultWindow::closeAfterArenaServerConfirmation()
{
	if(arenaQueryID == QueryID::NONE)
		return;
	logGlobal->info("[ARENA] closing server-confirmed battle result query=%d", arenaQueryID.getNum());
	resultSubmitted = true;
	CPlayerInterface & intTmp = owner;
	close();
	if(ENGINE->windows().topWindow<BattleWindow>())
