...
std::vector<AdventureMapShortcutState> AdventureMapShortcuts::getShortcuts()
{
	...
	std::vector<AdventureMapShortcutState> result = {
		...
		{ EShortcut::ADVENTURE_QUEST_LOG,        arenaMode ? optionCanOpenArenaTavern() : optionCanViewQuests(),  [this, arenaMode]() { arenaMode ? this->showArenaTavern() : this->showQuestlog(); } },
...
void AdventureMapShortcuts::showArenaTavern()
{
	for(const auto * town : GAME->interface()->cb->getTownsInfo(true))
	{
...
void AdventureMapShortcuts::endTurn()
{
	...
	// Arena deployment is an explicit ready phase, not a regular adventure-map
	// turn. Ending it must immediately mark the player ready, without the
	// original "hero still has movement points" confirmation dialog.
	if(GAME->interface()->cb->getMapHeader()->battleOnly && GAME->interface()->arenaDeploymentActive.load())
	{
...
void AdventureMapShortcuts::toMainMenu()
{
	GAME->interface()->showYesNoDialog(
		...
		[]()
		{
			GAME->server().requestGameAbandon();
		},
		0
		);
}
...
void AdventureMapShortcuts::openObject()
{
	...
	if(t)
		GAME->interface()->requestTownWindow(t);
}
...
bool AdventureMapShortcuts::optionCanOpenArenaTavern()
{
	if(!optionInMapView() || GAME->interface()->cb->howManyHeroes(true) >= 3)
		return false;

	const auto towns = GAME->interface()->cb->getTownsInfo(true);
	return std::any_of(towns.begin(), towns.end(), [](const CGTownInstance * town)
	{
		return town->hasBuilt(BuildingID::TAVERN);
	});
}
