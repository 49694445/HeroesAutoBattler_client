	...
	, mapLevel(0)
{
	...
	{
		if(auto button = widget<CButton>("buttonQuestLog"))
		{
			button->setTextOverlay(LIBRARY->generaltexth->translate("vcmi.arena.tavern.button"), FONT_SMALL, Colors::YELLOW);
			button->setHelp(CButton::tooltipLocalized("vcmi.arena.tavern.open"));
		}
	}
...
std::shared_ptr<CIntObject> AdventureMapWidget::getStatusBar()
{
	return widget<CIntObject>("statusBar");
}
