...
#include "../windows/ArenaHeroSnapshotWindow.h"
...
namespace
{
/// Adventure map children are normally repainted through show(). CLabel only
/// implements showAll(), so a plain label is erased by subsequent map updates.
/// Keep this overlay persistent in the same way as TurnTimerWidget.
class ArenaNextBattleTerrainLabel final : public CLabel
{
public:
	using CLabel::CLabel;
	...
	backgroundDimLevel(settings["adventure"]["backgroundDimLevel"].Integer())
{
	...
	arenaNextBattleTerrainLabel = std::make_shared<ArenaNextBattleTerrainLabel>(
		0,
		0,
		FONT_SMALL,
		...
		Colors::YELLOW,
		"");
	arenaNextBattleTerrainLabel->setAutoRedraw(false);
	arenaNextBattleTerrainLabel->disable();
	refreshArenaNextBattleTerrainLabel();
	refreshArenaPreviousEnemyHeroPortrait();
	
	addUsedEvents(KEYBOARD | TIME);
}

void AdventureMapInterface::ensureArenaDeploymentTimer()
{
	if(watches || !GAME->interface() || !GAME->interface()->cb->getMapHeader()->battleOnly)
		return;
	OBJECT_CONSTRUCTION_TARGETED(this);
	watches = std::make_shared<TurnTimerWidget>(Point(24, 24));
}

void AdventureMapInterface::refreshArenaNextBattleTerrainLabel()
{
	if(!arenaNextBattleTerrainLabel)
		return;

	auto * interface = GAME->interface();
	const bool battleOnly = interface && interface->cb && interface->cb->getMapHeader()->battleOnly;
	const bool deploymentActive = interface && interface->arenaDeploymentActive.load();
	const bool shouldShow = battleOnly && deploymentActive;
	const bool siege = shouldShow && interface->isArenaNextBattleSiege();
	const PlayerColor lastOrdinaryLoser = shouldShow ? interface->getArenaLastOrdinaryBattleLoser() : PlayerColor::NEUTRAL;
	const int32_t consecutiveOrdinaryLosses = shouldShow ? interface->getArenaConsecutiveOrdinaryLosses() : 0;
	const int32_t siegeInterval = shouldShow ? interface->getArenaSiegeInterval() : 1;
	const TerrainId terrainID = shouldShow ? interface->getArenaNextBattleTerrain() : TerrainId::NONE;
	const auto * terrain = terrainID != TerrainId::NONE ? LIBRARY->terrainTypeHandler->getById(terrainID) : nullptr;
	const auto statusBar = widget ? widget->getStatusBar() : nullptr;
	const Rect statusBarArea = statusBar ? statusBar->pos : Rect();
	const Point labelPosition(
		statusBarArea.x + 78,
		statusBarArea.y - statusBarArea.h - 2);
	const int32_t labelMaxWidth = std::max(0, statusBarArea.w - 86);
	const bool canDisplay = (siege || terrain) && statusBar && labelMaxWidth > 0;

	if(!canDisplay)
	{
		if(!arenaNextBattleTerrainLabel->isDisabled())
		{
			arenaNextBattleTerrainLabel->clear();
			arenaNextBattleTerrainLabel->disable();
			redraw();
		}
		return;
	...
	arenaNextBattleTerrainLabel->setMaxWidth(labelMaxWidth);
	arenaNextBattleTerrainLabel->moveTo(labelPosition);
	std::string battleHint;
	if(consecutiveOrdinaryLosses > 0 && (lastOrdinaryLoser == PlayerColor(0) || lastOrdinaryLoser == PlayerColor(1)))
	{
		const PlayerColor winner = lastOrdinaryLoser == PlayerColor(0) ? PlayerColor(1) : PlayerColor(0);
		const std::string winKey = winner == PlayerColor(0)
			? "vcmi.arena.ordinaryWinStreak.red"
			: "vcmi.arena.ordinaryWinStreak.blue";
		const int32_t winsUntilSiege = std::max(0, siegeInterval - consecutiveOrdinaryLosses);
		battleHint = boost::str(boost::format(LIBRARY->generaltexth->translate(winKey))
			% consecutiveOrdinaryLosses % winsUntilSiege);
	}
	battleHint += siege
		? LIBRARY->generaltexth->translate("vcmi.arena.nextBattleSiege")
		: LIBRARY->generaltexth->translate("vcmi.arena.nextBattleTerrain") + terrain->getNameTranslated();
	arenaNextBattleTerrainLabel->setText(battleHint);
	if(arenaNextBattleTerrainLabel->isDisabled())
		arenaNextBattleTerrainLabel->enable();
	moveChildForeground(arenaNextBattleTerrainLabel.get());
	redraw();
}

void AdventureMapInterface::refreshArenaPreviousEnemyHeroPortrait()
{
	auto * interface = GAME->interface();
	const bool deploymentActive = interface && interface->cb
		&& interface->cb->getMapHeader()->battleOnly
		&& interface->arenaDeploymentActive.load();
	const auto * snapshot = deploymentActive && interface->getArenaPreviousEnemyHero()
		? &*interface->getArenaPreviousEnemyHero()
		: nullptr;

	if(!snapshot)
	{
		if(arenaPreviousEnemyHeroPortrait && !arenaPreviousEnemyHeroPortrait->isDisabled())
		{
			arenaPreviousEnemyHeroPortrait->disable();
			redraw();
		}
		return;
	}

	// The map view is the actual terrain viewport.  Anchoring to the full
	// AdventureMapInterface would place this over the right-hand side panel.
	constexpr int32_t portraitWidth = 64;
	constexpr int32_t viewportMargin = 6;
	const Rect terrain = terrainAreaPixels();
	const Point anchor(
		std::max(terrain.x + viewportMargin, terrain.x + terrain.w - portraitWidth - viewportMargin),
		terrain.y + viewportMargin);

	if(!arenaPreviousEnemyHeroPortrait)
	{
		OBJECT_CONSTRUCTION_TARGETED(this);
		arenaPreviousEnemyHeroPortrait = std::make_shared<ArenaHeroSnapshotPortrait>(anchor, *snapshot, LIBRARY->generaltexth->translate("vcmi.arena.snapshot.enemyTitle"));
	}
	else
	{
		arenaPreviousEnemyHeroPortrait->setSnapshot(*snapshot);
		arenaPreviousEnemyHeroPortrait->moveTo(anchor);
		if(arenaPreviousEnemyHeroPortrait->isDisabled())
			arenaPreviousEnemyHeroPortrait->enable();
	}
	moveChildForeground(arenaPreviousEnemyHeroPortrait.get());
	redraw();
}
...
{
	CIntObject::activate();
	// Modal windows and town screens deactivate this interface. Reassert the
	// persistent snapshot overlay whenever the map becomes visible again.
	refreshArenaPreviousEnemyHeroPortrait();
...
void AdventureMapInterface::hotkeyEndingTurn()
{
	...
	// Dedicated arena servers own their day-start recovery snapshot and reject
	// client SaveGame packets. Sending an autosave immediately before the arena
	// deployment-ready request is redundant and can stall that request behind a
	// blocked save packet. Ordinary maps keep their existing autosave behavior.
	const bool arenaDeployment = GAME->interface()->cb->getMapHeader()->battleOnly
		&& GAME->interface()->arenaDeploymentActive.load();
	if(!arenaDeployment && !settings["general"]["startTurnAutosave"].Bool())
	{
	...
	if(arenaDeployment)
		GAME->interface()->cb->arenaDeploymentReady();
	else
...
void AdventureMapInterface::onTileLeftClicked(const int3 &targetPosition)
{
	...
	if(GAME->interface()->localState->getCurrentArmy()->ID != Obj::HERO) //hero is not selected (presumably town)
	{
		if(GAME->interface()->localState->getCurrentArmy() == topBlocking) //selected town clicked
			GAME->interface()->requestTownWindow(static_cast<const CGTownInstance*>(topBlocking));
		else if(canSelect)
