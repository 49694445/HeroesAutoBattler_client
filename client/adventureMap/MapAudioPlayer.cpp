...
{
	if(audioPlaying && playerMakingTurn)
	{
		const auto arenaTerrain = GAME->interface()->getArenaNextBattleTerrain();
		if(GAME->interface()->arenaDeploymentActive.load() && arenaTerrain != TerrainId::NONE)
		{
			const auto * terrain = LIBRARY->terrainTypeHandler->getById(arenaTerrain);
			if(terrain)
				ENGINE->music().playMusicFromSet("terrain", terrain->getJsonKey(), true, false);
			return;
		}

		const auto * tile = currentSelection ? GAME->interface()->cb->getTile(currentSelection->visitablePos()) : nullptr;
