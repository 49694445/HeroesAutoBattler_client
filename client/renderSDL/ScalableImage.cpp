...
void ScalableImageShared::draw(SDL_Surface * where, const Point & dest, const Rect * src, const ScalableImageParameters & parameters, int scalingFactor)
{
	...
	const int playerImageIndex = 1 + parameters.player.getNum();
	const bool hasPlayerImageIndex = parameters.player != PlayerColor::CANNOT_DETERMINE
		&& playerImageIndex >= 0
		&& playerImageIndex < static_cast<int>(scaled.at(scalingFactor).playerColored.size());
	bool playerLoading = hasPlayerImageIndex
		&& scaled.at(scalingFactor).playerColored.at(playerImageIndex)
		&& scaled.at(scalingFactor).playerColored.at(playerImageIndex)->isLoading();
	...
	if (hasPlayerImageIndex && scaled.at(scalingFactor).playerColored.at(playerImageIndex))
	{
		scaled.at(scalingFactor).playerColored.at(playerImageIndex)->draw(where, parameters.palette, dest, src, Colors::WHITE_TRUE, parameters.alphaValue, locator.layer);
	}
	else
	{
