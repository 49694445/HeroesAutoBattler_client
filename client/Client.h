...
public:
	...
	/// Public arena health is intentionally cached outside permission-filtered callbacks.
	void updateArenaHealth(const SetArenaHealth & pack);
	std::optional<int32_t> getArenaHealth(PlayerColor player) const;
	void updateArenaBattleRecruitmentThresholds(const SetArenaBattleCreatureProgress & pack);
	std::vector<ArenaRecruitmentThresholdObservation> getArenaBattleRecruitmentThresholds(BattleID battleID, PlayerColor observedPlayer) const;
	void clearArenaBattleRecruitmentThresholds(BattleID battleID);
	...
	JsonNode prepareLocalStateForSave(PlayerColor player) override;
...
private:
	...
	mutable std::mutex arenaHealthMutex;
	std::map<PlayerColor, int32_t> arenaHealthCache;
	mutable std::mutex arenaBattleRecruitmentThresholdMutex;
	std::map<BattleID, std::map<PlayerColor, std::map<CreatureID, EArenaRecruitmentThresholdState>>> arenaBattleRecruitmentThresholdCache;
	void restoreBattleAILocalState();
