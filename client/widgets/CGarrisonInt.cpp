...
namespace
{
bool hasArenaRecruitmentExperience(const CCreature * creature)
{
	if(!creature || !GAME->interface() || !GAME->interface()->cb->getMapHeader()->battleOnly)
		return false;
	...
	const auto experience = playerState->arenaCreatureExperience.find(creature->getId());
	const auto threshold = playerState->arenaRecruitThreshold.find(creature->getId());
	return experience != playerState->arenaCreatureExperience.end()
		&& threshold != playerState->arenaRecruitThreshold.end()
		&& experience->second >= threshold->second;
}

const PlayerState * getLocalArenaMainHeroState(const CArmedInstance * army)
{
	const auto * interface = GAME->interface();
	const auto * hero = dynamic_cast<const CGHeroInstance *>(army);
	if(!interface || !hero || !interface->cb->getMapHeader()->battleOnly || !interface->arenaDeploymentActive.load())
		return nullptr;

	const auto * playerState = interface->cb->getPlayerState(interface->playerID);
	return playerState && playerState->arenaMainHero == hero->id && hero->getOwner() == interface->playerID
		? playerState
		: nullptr;
}

std::vector<CreatureID> getLocalArenaCreatureFamily(const PlayerState * playerState, CreatureID creature)
{
	if(playerState)
		for(const auto * town : playerState->getTowns())
			for(const auto & slot : town->getTown()->creatures)
				if(std::any_of(slot.begin(), slot.end(), [creature](const auto & entry){ return entry.toCreature()->getId() == creature; }))
			{
				std::vector<CreatureID> family;
				for(const auto & entry : slot)
					family.push_back(entry.toCreature()->getId());
				return family;
			}
	return {creature};
}

void drawArenaRecruitmentArrow(Canvas & canvas, const Point & position)
{
	canvas.drawText(position, FONT_MEDIUM, Colors::GREEN, ETextAlignment::TOPLEFT, "▲");
}
}

std::string getCreatureDismissConfirmationText(const CArmedInstance * army)
{
	if(getLocalArenaMainHeroState(army))
		return LIBRARY->generaltexth->translate("vcmi.arena.dismissCreatureFamily.confirm");

	return LIBRARY->generaltexth->allTexts[12];
}
...
void CGarrisonSlot::show(Canvas & to)
{
	CIntObject::show(to);
	if(hasArenaRecruitmentExperience(creature))
		drawArenaRecruitmentArrow(to, pos.topLeft() + Point(pos.w - 15, 0));
}
...
{
	const auto * army = getObj();
	bool canDismiss = army && army->tempOwner == GAME->interface()->playerID
		&& (army->stacksCount() > 1 || !army->needsLastStack());

	if(canDismiss)
		if(const auto * playerState = getLocalArenaMainHeroState(army))
		{
			const auto family = getLocalArenaCreatureFamily(playerState, myStack->getCreatureID());
			const auto * hero = dynamic_cast<const CGHeroInstance *>(army);
			if(hero && hero->getFactionID() == FactionID::NECROPOLIS
				&& vstd::contains(family, CreatureID::SKELETON))
				canDismiss = false;
			const bool allStacksBelongToFamily = std::all_of(army->stacks.begin(), army->stacks.end(), [&family](const auto & entry)
			{
				return vstd::contains(family, entry.second->getCreatureID());
			});
			if(army->needsLastStack() && allStacksBelongToFamily)
				canDismiss = false;
		}
...
bool CGarrisonSlot::viewInfo()
{
	...
	const auto * hero = dynamic_cast<const CGHeroInstance *>(getObj());
	const bool arenaDeployment = GAME->interface()->cb->getMapHeader()->battleOnly
		&& GAME->interface()->arenaDeploymentActive.load()
		&& hero
		&& hero->tempOwner == GAME->interface()->playerID;

	if(arenaDeployment)
	{
		hero->fillUpgradeInfo(pom, *myStack);
		const auto * playerState = GAME->interface()->cb->getPlayerState(GAME->interface()->playerID);
		if(playerState)
		{
			for(const auto * town : playerState->getTowns())
				town->fillUpgradeInfo(pom, *myStack);
		}
	}
	else
	{
...
void CGarrisonSlot::clickPressed(const Point & cursorPosition)
{
		bool refr = false;
		const CGarrisonSlot * selection = owner->getSelection();
		if(selection && selection != this && selection->upg != upg && !owner->transfersEnabled)
		{
			// Virtual visits allow inspection but never town/hero troop transfer.
			owner->selectSlot(nullptr);
			owner->redraw();
			return;
		}
...
bool CGarrisonSlot::handleSplittingShortcuts()
{
	...
	// A modifier click can arrive while the arena is still opening and before
	// this player has an active turn. Do not queue a troop mutation that will be
	// applied later against a different stack layout.
	if(!GAME->interface()->makingTurn)
		return true;
	...
	else if(isLShift && isAlt)
	{
		...
		if(dismiss)
			GAME->interface()->showYesNoDialog(getCreatureDismissConfirmationText(getObj()), dismiss, nullptr);
	}
	else if(isAlt)
	{
...
void CGarrisonInt::bulkMergeStacks(const CGarrisonSlot * selected)
{
	...
	// The authoritative reply is asynchronous. Clear the selection immediately
	// so a rapid second Alt-click cannot emit ArrangeStacks from the stale layout
	// that existed before the bulk merge.
	selectSlot(nullptr);
	redraw();
}
