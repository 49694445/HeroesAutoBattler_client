...
#include "../../../lib/networkPacks/ArenaDeploymentMarketRules.h"
...
{
	const bool arenaDeployment = GAME->interface()->arenaDeploymentActive.load();
	if(arenaDeployment && (!bidTradePanel->isHighlighted() || !offerTradePanel->isHighlighted()
		|| !isArenaDeploymentResourceTrade(GameResID(bidTradePanel->getHighlightedItemId()),
			GameResID(offerTradePanel->getHighlightedItemId()))))
		return;
...
void CMarketResources::highlightingChanged()
{
	if(bidTradePanel->isHighlighted() && offerTradePanel->isHighlighted())
	{
		...
		const bool arenaDeployment = GAME->interface()->arenaDeploymentActive.load();
		const bool invalidArenaDirection = arenaDeployment
			&& !isArenaDeploymentResourceTrade(GameResID(bidTradePanel->getHighlightedItemId()),
				GameResID(offerTradePanel->getHighlightedItemId()));
		const bool isControlsBlocked = bidTradePanel->getHighlightedItemId() == offerTradePanel->getHighlightedItemId()
			|| invalidArenaDirection;
		offerSlider->block(isControlsBlocked);
		maxAmount->block(isControlsBlocked);
		deal->block(isControlsBlocked || (!GAME->interface()->makingTurn && !arenaDeployment));
	}
