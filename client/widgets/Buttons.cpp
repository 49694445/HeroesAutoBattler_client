...
void ButtonBase::update()
{
	...
	if (image)
	{
		const size_t frameCount = image->size();
		if(frameCount > 0)
		{
			const auto isAvailable = [frameCount](int frame)
			{
				return frame >= 0 && static_cast<size_t>(frame) < frameCount;
			};

			int targetFrame = stateToIndex[vstd::to_underlying(state)];
			if(!isAvailable(targetFrame))
			{
				// Some localized button DEFs contain no dedicated disabled frame.
				// Keep the button logically blocked, but render its normal frame when
				// possible. Other missing state frames use the last available frame.
				const int normalFrame = stateToIndex[vstd::to_underlying(EButtonState::NORMAL)];
				if(state == EButtonState::BLOCKED && isAvailable(normalFrame))
					targetFrame = normalFrame;
				else
					targetFrame = static_cast<int>(frameCount - 1);
			}
			image->setFrame(static_cast<size_t>(targetFrame));
		}
	}
