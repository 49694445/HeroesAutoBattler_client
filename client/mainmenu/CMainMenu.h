...
public:
	...
	static void openAuthenticatedArenaLobby(ELoadMode loadMode);
...
class CLoginScreen : public WindowBase
{
	std::shared_ptr<CFilledTexture> background;
	std::shared_ptr<CTextBox> title;
	std::shared_ptr<CLabel> usernameLabel;
	std::shared_ptr<CLabel> passwordLabel;
	std::shared_ptr<CLabel> usernameMarker;
	std::shared_ptr<CLabel> passwordMarker;
	std::shared_ptr<CLabel> rememberPasswordLabel;
	struct ServerChoice
	{
		std::string id;
		std::string name;
	...
	std::vector<ServerChoice> serverChoices;
	std::shared_ptr<ComboBox> serverDropdown;
	std::shared_ptr<CButton> buttonLogin;
	std::shared_ptr<CToggleButton> rememberPassword;
	std::shared_ptr<CTextInput> username;
	std::shared_ptr<CTextInput> password;
	bool pendingLogin = false;

	void selectServerTarget(int target);
	void blockServerTarget(bool block);
	void updateButton();
	void submit();
public:
	CLoginScreen();
	...
	void onConnected();
	void onAuthenticationFailed(const std::string & reason);
	void onAuthenticationAccepted();
};
