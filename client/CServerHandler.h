...
enum class EClientState : ui8
{
	...
	AUTHENTICATING, // Connected, waiting for account authentication
	AUTH_REJECTED, // Server will close connection after rejected authentication
	...
	ABANDONING_GAME, // Waiting for authoritative PlayerEndsGame after conceding
...
class CServerHandler final : public IServerAPI, public LobbyInfo, public INetworkClientListener, boost::noncopyable
{
	friend class ApplyOnLobbyHandlerNetPackVisitor;

	enum class ETimerPurpose
	{
		REDIRECT_RETRY,
		AUTHENTICATION_TIMEOUT
	};

	class TimerListener final : public INetworkTimerListener
	{
		CServerHandler & owner;
		ETimerPurpose purpose;

	public:
		TimerListener(CServerHandler & owner, ETimerPurpose purpose);
	...
	EAccountRole accountRole = EAccountRole::PLAYER;
	TimerListener redirectRetryTimer;
	TimerListener authenticationTimeoutTimer;
	bool authenticationTimeoutScheduled = false;
	std::chrono::steady_clock::time_point authenticationTimeoutDeadline;
	...
	void onRedirectRetryTimer();
	void onAuthenticationTimeoutTimer();
	void scheduleAuthenticationTimeout();
	...
	std::string authenticatedPassword;
	bool serverRedirectPending = false;
	bool serverRedirectRetryScheduled = false;
	ui16 serverRedirectPort = 0;
	int serverRedirectAttempts = 0;
	std::string serverRedirectMatchId;
	std::string serverRedirectToken;

	void connectToServerInternal(const std::string & addr, ui16 port, bool persistEndpoint);
	void connectToRedirectServer();
	void retryPublicMatchmakingAfterRejectedRedirect();
...
public:
	...
	void prepareStateForLobby(EStartMode mode, ESelectionScreen screen, EServerMode serverMode, const std::vector<std::string> & playerNames);
	...
	EAccountRole getAccountRole() const;
	...
	void sendAuthentication(const std::string & username, const std::string & password);
	void handleAuthenticationResult(const LobbyAuthResult & result);
	void handleServerRedirect(const LobbyServerRedirect & redirect);
	void sendClientDisconnecting() override;
	void requestGameAbandon();
	...
	void setPlayerReady(bool ready) const;
	...
	void debugStartTest(std::string filename, bool save = false, int32_t difficultyIndex = -1);
