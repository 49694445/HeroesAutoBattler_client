...
void ApplyOnLobbyHandlerNetPackVisitor::visitLobbyQuickLoadGame(LobbyQuickLoadGame & pack)
{
	...
	if((state != EClientState::GAMEPLAY && state != EClientState::LOBBY && state != EClientState::NONE)
		|| !handler.logicConnection || !handler.si || handler.si->mode != EStartMode::LOAD_GAME)
	{
		logNetwork->warn("Ignoring unsynchronized quick-load request in client state %d", static_cast<int>(state));
		return;
	}

	// The dedicated server initiates reconnect reloads after authenticating a
	// frozen session participant. The client deliberately does not inspect the
	// transmitted save path: it only returns to lobby serialization and waits
	// for the server-authoritative LobbyStartGame that follows.
	handler.restartGameplay();
	handler.setState(EClientState::LOBBY);
	handler.sendStartGame(false, false);
}
...
void ApplyOnLobbyHandlerNetPackVisitor::visitLobbyClientConnected(LobbyClientConnected & pack)
{
	...
	if(isLocalClient)
	{
		...
		{
			const bool supportedScreen = handler.screenType == ESelectionScreen::newGame
				|| handler.screenType == ESelectionScreen::loadGame
				|| handler.screenType == ESelectionScreen::campaignList;
			if(!supportedScreen)
			{
				logGlobal->error("Refusing to create lobby screen for unsupported selection screen type %d", static_cast<int>(handler.screenType));
				handler.sendClientDisconnecting();
				return;
			}
		...
		handler.setState(EClientState::LOBBY);
		if(handler.loadMode == ELoadMode::SINGLE)
			handler.setPlayer(PlayerColor(1));
	}
}

void ApplyOnLobbyHandlerNetPackVisitor::visitLobbyAuthResult(LobbyAuthResult & pack)
{
	handler.handleAuthenticationResult(pack);
	result = false;
}

void ApplyOnLobbyHandlerNetPackVisitor::visitLobbyServerRedirect(LobbyServerRedirect & pack)
{
	handler.handleServerRedirect(pack);
	result = false;
}
