...
namespace fuzzylite {
    ...
    std::string FllExporter::toString(const Variable* variable) const {
        ...
        result.push_back(_indent + "range: " + Op::join(2, " ", variable->getMinimum(), variable->getMaximum()));
        result.push_back(_indent + "lock-range: " + (variable->isLockValueInRange() ? "true" : "false"));
    ...
    std::string FllExporter::toString(const InputVariable* inputVariable) const {
        ...
        result.push_back(
            _indent + "range: " + Op::join(2, " ", inputVariable->getMinimum(), inputVariable->getMaximum())
        );
        result.push_back(_indent + "lock-range: " + (inputVariable->isLockValueInRange() ? "true" : "false"));
    ...
    std::string FllExporter::toString(const OutputVariable* outputVariable) const {
        ...
        result.push_back(
            _indent + "range: " + Op::join(2, " ", outputVariable->getMinimum(), outputVariable->getMaximum())
        );
        result.push_back(_indent + "lock-range: " + (outputVariable->isLockValueInRange() ? "true" : "false"));
        result.push_back(_indent + "aggregation: " + toString(outputVariable->fuzzyOutput()->getAggregation()));
        result.push_back(_indent + "defuzzifier: " + toString(outputVariable->getDefuzzifier()));
        result.push_back(_indent + "default: " + Op::str(outputVariable->getDefaultValue()));
        result.push_back(_indent + "lock-previous: " + (outputVariable->isLockPreviousValue() ? "true" : "false"));
    ...
    std::string FllExporter::toString(const RuleBlock* ruleBlock) const {
        ...
        result.push_back(_indent + "enabled: " + (ruleBlock->isEnabled() ? "true" : "false"));
    ...
    std::string FllExporter::toString(const Activation* activation) const {
        if (not activation)
            return "none";
        if (activation->parameters().empty())
