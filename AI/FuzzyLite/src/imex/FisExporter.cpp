...
namespace fuzzylite {
    ...
    std::string FisExporter::exportSystem(const Engine* engine) const {
        ...
        for (std::size_t i = 0; i < engine->numberOfRuleBlocks(); ++i) {
            ...
            if (not conjunction)
                conjunction = rb->getConjunction();
            if (not disjunction)
                disjunction = rb->getDisjunction();
            if (not implication)
                implication = rb->getImplication();
        }
    ...
    std::string FisExporter::exportRule(const Rule* rule, const Engine* engine) const {
        ...
        if (operators.size() == 0)
            fis << "1";  // does not matter
        else if (operators.at(0)->name == Rule::andKeyword())
            fis << "1";
        else if (operators.at(0)->name == Rule::orKeyword())
            fis << "2";
        else
            fis << operators.at(0)->name;
        return fis.str();
    }
            ...
            for (std::size_t ixProposition = 0; ixProposition < propositions.size(); ++ixProposition) {
                ...
                for (std::size_t ixHedge = 0; ixHedge < hedges.size(); ++ixHedge) {
                    ...
                    if (hedge->name() == Not().name())
                        negated *= -1;
                    else if (hedge->name() == Extremely().name())
                        plusHedge += 0.3;
                    else if (hedge->name() == Very().name())
                        plusHedge += 0.2;
                    else if (hedge->name() == Somewhat().name())
                        plusHedge += 0.05;
                    else if (hedge->name() == Seldom().name())
                        plusHedge += 0.01;
                    else if (hedge->name() == Any().name())
                        plusHedge += 0.99;
                    else
                        plusHedge = fl::nan;  // Unreconized hedge combination (e.g. Any)
                }
    ...
    std::string FisExporter::toString(const TNorm* tnorm) const {
        if (not tnorm)
            return "";
        if (tnorm->className() == Minimum().className())
            return "min";
        if (tnorm->className() == AlgebraicProduct().className())
            return "prod";
        if (tnorm->className() == BoundedDifference().className())
            return "bounded_difference";
        if (tnorm->className() == DrasticProduct().className())
            return "drastic_product";
        if (tnorm->className() == EinsteinProduct().className())
            return "einstein_product";
        if (tnorm->className() == HamacherProduct().className())
            return "hamacher_product";
        if (tnorm->className() == NilpotentMinimum().className())
    ...
    std::string FisExporter::toString(const SNorm* snorm) const {
        if (not snorm)
            return "";
        if (snorm->className() == Maximum().className())
            return "max";
        if (snorm->className() == AlgebraicSum().className())
            return "probor";
        if (snorm->className() == BoundedSum().className())
            return "bounded_sum";
        if (snorm->className() == NormalizedSum().className())
            return "normalized_sum";
        if (snorm->className() == DrasticSum().className())
            return "drastic_sum";
        if (snorm->className() == EinsteinSum().className())
            return "einstein_sum";
        if (snorm->className() == HamacherSum().className())
            return "hamacher_sum";
        if (snorm->className() == NilpotentMaximum().className())
            return "nilpotent_maximum";
        if (snorm->className() == UnboundedSum().className())
            return "sum";
    ...
    std::string FisExporter::toString(const Term* term) const {
        ...
        if (const Bell* x = dynamic_cast<const Bell*>(term)) {
            ss << "'gbellmf',[" << Op::join(3, " ", x->getWidth(), x->getSlope(), x->getCenter()) << "]";
            return ss.str();
        }

        if (const Binary* x = dynamic_cast<const Binary*>(term)) {
            ss << "'binarymf,[" << Op::join(2, " ", x->getStart(), x->getDirection()) << "]";
            return ss.str();
        }

        if (const Concave* x = dynamic_cast<const Concave*>(term)) {
            ss << "'concavemf',[" << Op::join(2, " ", x->getInflection(), x->getEnd()) << "]";
            return ss.str();
        }
        ...
        if (const Cosine* x = dynamic_cast<const Cosine*>(term)) {
            ss << "'cosinemf',[" << Op::join(2, " ", x->getCenter(), x->getWidth()) << "]";
            return ss.str();
        }
        ...
        if (const Gaussian* x = dynamic_cast<const Gaussian*>(term)) {
            ss << "'gaussmf',[" << Op::join(2, " ", x->getStandardDeviation(), x->getMean()) << "]";
            return ss.str();
        }

        if (const GaussianProduct* x = dynamic_cast<const GaussianProduct*>(term)) {
            ss << "'gauss2mf',["
               << Op::join(4, " ", x->getStandardDeviationA(), x->getMeanA(), x->getStandardDeviationB(), x->getMeanB())
               << "]";
            return ss.str();
        }
        ...
        if (const PiShape* x = dynamic_cast<const PiShape*>(term)) {
            ss << "'pimf',["
               << Op::join(4, " ", x->getBottomLeft(), x->getTopLeft(), x->getTopRight(), x->getBottomRight()) << "]";
            return ss.str();
        }

        if (const Ramp* x = dynamic_cast<const Ramp*>(term)) {
            ss << "'rampmf',[" << Op::join(2, " ", x->getStart(), x->getEnd()) << "]";
            return ss.str();
        }

        if (const Rectangle* x = dynamic_cast<const Rectangle*>(term)) {
            ss << "'rectmf',[" << Op::join(2, " ", x->getStart(), x->getEnd()) << "]";
            return ss.str();
        }

        if (const SigmoidDifference* x = dynamic_cast<const SigmoidDifference*>(term)) {
            ss << "'dsigmf',[" << Op::join(4, " ", x->getRising(), x->getLeft(), x->getFalling(), x->getRight()) << "]";
            return ss.str();
        }

        if (const Sigmoid* x = dynamic_cast<const Sigmoid*>(term)) {
            ss << "'sigmf',[" << Op::join(2, " ", x->getSlope(), x->getInflection()) << "]";
            return ss.str();
        }

        if (const SigmoidProduct* x = dynamic_cast<const SigmoidProduct*>(term)) {
            ss << "'psigmf',[" << Op::join(4, " ", x->getRising(), x->getLeft(), x->getFalling(), x->getRight()) << "]";
            return ss.str();
        }

        if (const SShape* x = dynamic_cast<const SShape*>(term)) {
            ss << "'smf',[" << Op::join(2, " ", x->getStart(), x->getEnd()) << "]";
            return ss.str();
        }

        if (const Spike* x = dynamic_cast<const Spike*>(term)) {
            ss << "'spikemf',[" << Op::join(2, " ", x->getCenter(), x->getWidth()) << "]";
            return ss.str();
        }

        if (const Trapezoid* x = dynamic_cast<const Trapezoid*>(term)) {
            ss << "'trapmf',[" << Op::join(4, " ", x->getVertexA(), x->getVertexB(), x->getVertexC(), x->getVertexD())
               << "]";
            return ss.str();
        }

        if (const Triangle* x = dynamic_cast<const Triangle*>(term)) {
            ss << "'trimf',[" << Op::join(3, " ", x->getVertexA(), x->getVertexB(), x->getVertexC()) << "]";
            return ss.str();
        }

        if (const ZShape* x = dynamic_cast<const ZShape*>(term)) {
            ss << "'zmf',[" << Op::join(2, " ", x->getStart(), x->getEnd()) << "]";
            return ss.str();
        }
