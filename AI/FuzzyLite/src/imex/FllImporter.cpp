...
namespace fuzzylite {
    ...
    FllImporter::FllImporter(const std::string& separator) : Importer(), _separator(separator) {}
    ...
    Engine* FllImporter::fromString(const std::string& code) const {
        ...
        while (std::getline(fllReader, line)) {
            ...
            if ("Engine" == key) {
                ...
                continue;
            } else if (key == "InputVariable" or key == "OutputVariable" or key == "RuleBlock") {
    ...
    void FllImporter::processInputVariable(const std::string& block, Engine* engine) const {
        ...
        while (std::getline(reader, line)) {
            ...
            if ("InputVariable" == keyValue.first) {
                ...
                throw Exception(
                    "[import error] key <" + keyValue.first
                        + "> not "
                          "recognized in pair <"
    ...
    void FllImporter::processRuleBlock(const std::string& block, Engine* engine) const {
        ...
        while (std::getline(reader, line)) {
            ...
            if ("RuleBlock" == keyValue.first) {
                ...
                try {
                    rule->load(engine);
                } catch (std::exception& ex) { FL_LOG(ex.what()); }
    ...
    Term* FllImporter::parseTerm(const std::string& text, Engine* engine) const {
        ...
        if (tokens.size() < 2) {
            throw Exception(
                "[syntax error] expected a term in format <name class parameters>, "
                "but found <"
                    + text + ">",
                FL_AT
            );
        ...
        if (parameters.size() > 1) {
            ...
            if (IntegralDefuzzifier* integralDefuzzifier = dynamic_cast<IntegralDefuzzifier*>(defuzzifier)) {
                ...
                if (parameter == "Automatic")
                    type = WeightedDefuzzifier::Automatic;
                else if (parameter == "TakagiSugeno")
                    type = WeightedDefuzzifier::TakagiSugeno;
                else if (parameter == "Tsukamoto")
                    type = WeightedDefuzzifier::Tsukamoto;
                else
                    throw Exception(
    ...
    bool FllImporter::parseBoolean(const std::string& boolean) const {
        ...
        if ("false" == boolean)
            return false;
        throw Exception(
            "[syntax error] expected boolean <true|false>, "
            "but found <"
                + boolean + ">",
            FL_AT
        );
    ...
    std::pair<std::string, std::string> FllImporter::parseKeyValue(const std::string& text, char separator) const {
        std::size_t half = text.find_first_of(separator);
        if (half == std::string::npos) {
            std::ostringstream ex;
            ex << "[syntax error] expected pair in the form "
                  "<key"
               << separator << "value>, but found <" << text << ">";
