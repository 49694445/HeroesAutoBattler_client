...
std::mutex arenaProbeDecisionMutex;

void appendJsonString(std::ostream & output, std::string_view value)
{
...
void recordArenaProbeDecision(const BattleID & battleID, int decisionIndex, const CPlayerBattleCallback * battle,
	Schema::IModel * model, const State * state, Schema::Action action)
{
	const char * outputPath = std::getenv("VCMI_ARENA_BATTLE_PROBE_MMAI_DECISIONS");
	if(!outputPath || !*outputPath || !battle || !model || !state)
		return;
	const auto * battleInfo = dynamic_cast<const BattleInfo *>(battle->getBattle());
	if(!battleInfo || battleInfo->arenaProbeScenarioId.empty())
		return;
	const auto * features = state->getBattlefieldState();
	const auto * actionMask = state->getActionMask();
	const auto * stochastic = dynamic_cast<const NNModelStochastic *>(model);
	if(!features || !actionMask || action < 0 || static_cast<size_t>(action) >= actionMask->size() || !actionMask->at(action))
		throw std::runtime_error("MMAI probe decision is missing a valid encoded observation");
	if(!stochastic)
		throw std::runtime_error("MMAI probe decision came from an unsupported non-stochastic model");

	// This is a hot path (one record per MMAI prediction). Building a JsonNode tree,
	// pretty-printing it, then stripping whitespace used to add tens of milliseconds
	// to every action. Stream the identical v1 JSON envelope directly instead.
	std::ostringstream record;
	record.imbue(std::locale::classic());
	record << std::setprecision(std::numeric_limits<float>::max_digits10);
	record << "{\"schemaVersion\":1,\"event\":\"mmai-decision\",\"scenarioId\":";
	appendJsonString(record, battleInfo->arenaProbeScenarioId);
	record << ",\"battleId\":" << battleID.getNum()
		<< ",\"decisionIndex\":" << decisionIndex << ",\"side\":";
	appendJsonString(record, battle->battleGetMySide() == BattleSide::ATTACKER ? "attacker" : "defender");
	record << ",\"controller\":{\"aiId\":\"vcmi:MMAI\",\"modelName\":";
	appendJsonString(record, model->getName());
	record << ",\"modelVersion\":" << model->getVersion() << ",\"modelPath\":";
	appendJsonString(record, stochastic->getPath());
	record << ",\"temperature\":" << stochastic->getTemperature()
		<< "},\"observation\":{\"schema\":\"MMAI:v13\",\"stateSize\":" << features->size()
		<< ",\"actionSize\":" << actionMask->size() << ",\"stateFeatures\":[";
	for(size_t index = 0; index < features->size(); ++index)
	{
		if(index)
			record.put(',');
		record << features->at(index);
	}
	record << "],\"legalActionMask\":[";
	for(size_t index = 0; index < actionMask->size(); ++index)
	{
		if(index)
			record.put(',');
		record << (actionMask->at(index) ? "true" : "false");
	}
	record << "]},\"chosenAction\":" << action << '}';

	std::scoped_lock lock(arenaProbeDecisionMutex);
	std::ofstream output(outputPath, std::ios::binary | std::ios::app);
	if(!output)
		throw std::runtime_error("Unable to append MMAI arena probe decision trace");
	output << record.str() << '\n';
	output.close();
	if(!output)
		throw std::runtime_error("Unable to write MMAI arena probe decision trace");
}
...
{
	ScopedLosslessFastResolveInference fastResolveScope(losslessFastResolve);
...
void BAI::_activeStack(const BattleID & bid, const CStack * astack)
{
	...
	if(!inFallback && getActionTotalCalls >= maxPredictions)
	{
		warn("Reached %d predictions, will fall back to BattleAI until this combat ends", maxPredictions);
		inFallback = true;
	}
	...
	while(true)
	{
		...
		if(ba && (a != Schema::ACTION_RETREAT || resetting))
		{
			...
			recordArenaProbeDecision(bid, arenaProbeDecisionIndex++, battle.get(), model, state.get(), a);
			errcounter = 0;
			cb->battleMakeUnitAction(bid, *ba);
			break;
		}
