		...
		for(int i = 0; i < EI(GlobalAction::_count); i++)
		{
			switch(static_cast<GlobalAction>(i))
			{
				case GlobalAction::RETREAT:
					// Retreat and surrender are strategic decisions handled before model
					// inference by makeSurrenderRetreatDecision(). Never expose RETREAT to
					// the neural policy after that decision declines it; arena battles in
					// particular prohibit both actions entirely.
					actmask.push_back(false);
					break;
