...
AllLinks Battlefield::InitAllLinks(const CPlayerBattleCallback * battle, const Stacks & stacks, const Queue & queue, std::shared_ptr<Hexes> & hexes)
{
	...
	// Adjacency is immutable for the fixed 165-hex battlefield. Build it once
	// in exactly the same source/destination order as the original all-pairs
	// loop, then reuse it without changing the model input bytes.
	static std::once_flag adjacentLinksOnce;
	static std::shared_ptr<Links> adjacentLinks;
	std::call_once(adjacentLinksOnce, [&hexes]()
	{
		auto links = std::make_shared<Links>();
		const auto adjmap = InitAdjMap();
		for(const auto & srcrow : *hexes)
			for(const auto & srchex : srcrow)
				for(const auto & dstrow : *hexes)
					for(const auto & dsthex : dstrow)
						if(adjmap.contains({srchex->bhex.toInt(), dsthex->bhex.toInt()}))
							links->add(srchex->id, dsthex->id, 1);
		adjacentLinks = std::move(links);
	});
	allLinks[LT::ADJACENT] = adjacentLinks;

	for(auto & srcrow : *hexes)
	{
		for(auto & srchex : srcrow)
		{
			// Every non-adjacency link requires a source stack. Empty source
			// hexes were only visited to construct the static adjacency links.
			if(!srchex->stack)
				continue;
			for(auto & dstrow : *hexes)
			{
				...
				{
					LinkTwoHexes(allLinks, battle, stacks, queue, srchex.get(), dsthex.get(), false);
				}
			}
		}
	}
