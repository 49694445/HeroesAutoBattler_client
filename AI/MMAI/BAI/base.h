...
public:
	...
	Create(Schema::IModel * model, const std::shared_ptr<Environment> & env, const std::shared_ptr<CBattleCallback> & cb,
		bool enableSpellsUsage, int maxPredictions, bool losslessFastResolve = false);

	Base() = delete;
	Base(Schema::IModel * model, int version, const std::shared_ptr<Environment> & env,
		const std::shared_ptr<CBattleCallback> & cb, int maxPredictions, bool losslessFastResolve = false);
	...
	const bool losslessFastResolve;
	...
	{
		if((!losslessFastResolve || level >= ELogLevel::WARN) && logAi->getEffectiveLevel() <= level)
