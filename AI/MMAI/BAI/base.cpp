...
// static
std::shared_ptr<Base>
Base::Create(Schema::IModel * model, const std::shared_ptr<Environment> & env,
	const std::shared_ptr<CBattleCallback> & cb, bool enableSpellsUsage, int maxPredictions, bool losslessFastResolve)
{
	std::shared_ptr<Base> res;
	auto version = model->getVersion();

	if(version == 13)
		res = std::make_shared<V13::BAI>(model, version, env, cb, maxPredictions, losslessFastResolve);
	else
...
Base::Base(Schema::IModel * model, int version, const std::shared_ptr<Environment> & env,
	const std::shared_ptr<CBattleCallback> & cb, int maxPredictions, bool losslessFastResolve)
	: model(model), version(version), name("BAI-v" + std::to_string(version)), colorname(cb->getPlayerID()->toString()),
	  env(env), cb(cb), maxPredictions(maxPredictions), losslessFastResolve(losslessFastResolve)
{
