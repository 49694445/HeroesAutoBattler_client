...
namespace
{
	...
	template<typename T>
	std::span<const T> tensorSpan1D(const std::string & name, const Ort::Value & tensor, int numel)
	{
		...
		return {tensor.GetTensorData<T>(), static_cast<size_t>(numel)};
	}

	template<typename T>
	const T * tensorData2D(const std::string & name, const Ort::Value & tensor, const std::pair<int64_t, int64_t> & dims)
	{
	...
	std::pair<Sample, Sample> categorical(std::span<const float> probs, float temperature, std::mt19937 & rng)
	{
		auto sample = Sample{};
		auto greedy = Sample{};
	...
	struct ScopedTimer
	{
		...
		~ScopedTimer()
		{
			if(losslessFastResolveInference)
				return;
...
const std::string & NNModelStochastic::getPath() const
{
	return path;
}

float NNModelStochastic::getTemperature() const
{
	return temperature;
}

std::string NNModelStochastic::exportRngState() const
{
...
bool NNModelStochastic::importRngState(const std::string & state)
{
...
void NNModelStochastic::resetRng(uint64_t seed)
{
	std::lock_guard lock(rngMutex);
	rng = std::mt19937(seed);
}
...
int NNModelStochastic::getAction(const MMAI::Schema::IState * s)
{
	...
	const auto act0_probs = tensorSpan1D<float>("act0_probs", outputs[0], 4); // WAIT, MOVE, AMOVE, SHOOT
	const auto * hex1_probs = tensorData2D<float>("hex1_probs", outputs[1], {4, 165});
	const auto * hex2_probs = tensorData2D<float>("hex2_probs", outputs[2], {165, 165});
	const auto act0_mask = tensorSpan1D<int>("act0_mask", outputs[3], 4); // WAIT, MOVE, AMOVE, SHOOT
	const auto * hex1_mask = tensorData2D<int>("hex1_mask", outputs[4], {4, 165});
	const auto * hex2_mask = tensorData2D<int>("hex2_mask", outputs[5], {165, 165});

	// Keep the three dependent draws atomic with respect to a concurrent save.
	// A snapshot taken between them would not represent any action boundary.
	std::lock_guard rngLock(rngMutex);
	const auto [act0_sample, act0_greedy] = categorical(act0_probs, temperature, rng);
	const auto [hex1_sample, hex1_greedy] = categorical(
		std::span<const float>(hex1_probs + act0_sample.index * 165, 165), temperature, rng);
	const auto [hex2_sample, hex2_greedy] = categorical(
		std::span<const float>(hex2_probs + hex1_sample.index * 165, 165), temperature, rng);

	if(act0_sample.prob == 0)
		throwf("getAction: act0_sample has 0 probability");
	else if(act0_mask[act0_sample.index] == 0)
	...
	if(act0_sample.index > 0)
	{
		...
		else if(hex1_mask[act0_sample.index * 165 + hex1_sample.index] == 0)
	...
	if(act0_sample.index == 2)
	{
		...
		else if(hex2_mask[hex1_sample.index * 165 + hex2_sample.index] == 0)
	...
	if(!losslessFastResolveInference && logAi->isDebugEnabled())
	{
...
std::vector<Ort::Value> NNModelStochastic::prepareInputsV13(const MMAI::Schema::IState * s, const MMAI::Schema::V13::ISupplementaryData * sup)
{
	...
	const bool logDebugInputs = !losslessFastResolveInference && logAi->isDebugEnabled();
	int i = 0;
